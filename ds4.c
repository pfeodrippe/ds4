/* =========================================================================
 * ds4.c - Qwen3-Coder-30B-A3B-Instruct inference engine.
 * =========================================================================
 *
 * This file is deliberately vertical: it owns GGUF loading, the fixed
 * Qwen3-Coder tensor layout, CPU reference kernels, the whole-model
...
#define DS4_ROPE_FREQ_BASE      (10000000.0f)
...
/* =========================================================================
 * Fixed Qwen3-Coder-30B-A3B-Instruct Shape.
 * =========================================================================
 *
 * These constants define the single model this program accepts.  The
...
enum {
    ...
    DS4_N_EXPERT           = 128,
    DS4_N_EXPERT_USED      = 8,
    DS4_N_EXPERT_ACTIVE    = 3,
    DS4_N_EXPERT_SHARED    = 0,
    DS4_N_FF               = 5472,
    DS4_N_FF_EXP           = 768,
    DS4_N_HASH_LAYER       = 0,
    DS4_N_SWA              = 0,
    ...
    DS4_N_HC               = 1,
    DS4_N_HC_SINKHORN_ITER = 0,
    DS4_EAGLE3_N_EXTRACT   = 3,
    DS4_EAGLE3_N_EMBD      = 2048,
    DS4_EAGLE3_N_FF        = 12288,
    DS4_EAGLE3_N_VOCAB     = 32000,
    DS4_EAGLE3_N_LAYER     = 1,
    DS4_MAX_DRAFT_TOKENS   = 16,
};

static const uint32_t DS4_EAGLE3_EXTRACT_LAYERS[DS4_EAGLE3_N_EXTRACT] = {
    2, 24, 45,
};
...
DS4_STATIC_ASSERT(ds4_block_q4_0_size, sizeof(block_q4_0) == 18);
...
static uint32_t ds4_layer_compress_ratio(uint32_t il) {
    if (il >= DS4_N_LAYER) ds4_die("Qwen3 layer index is outside the fixed model layout");
    return 0;
}
...
static bool ds4_strstr(ds4_str haystack, const char *needle) {
    const size_t n = strlen(needle);
    if (n == 0 || n > haystack.len) return false;
    for (uint64_t i = 0; i <= haystack.len - n; i++) {
...
static void cpu_directional_steering_project_rows_multi(
        float        *x,
        int           n,
        const float **dirs,
        uint32_t      il,
...
static void model_summary(const ds4_model *m) {
    ...
    model_get_u32(m, "qwen3moe.block_count", &layers);
    model_get_u64(m, "qwen3moe.context_length", &ctx_train);
    model_get_u32(m, "qwen3moe.attention.head_count", &n_head);
    model_get_u32(m, "qwen3moe.attention.head_count_kv", &n_head_kv);
    model_get_u32(m, "qwen3moe.attention.key_length", &head_dim);
    model_get_u32(m, "qwen3moe.attention.sliding_window", &n_swa);
    model_get_u32(m, "qwen3moe.expert_count", &n_expert);
    model_get_u32(m, "qwen3moe.expert_used_count", &n_expert_used);
    model_get_u32(m, "qwen3moe.expert_group_count", &n_expert_groups);
    model_get_u32(m, "qwen3moe.expert_group_used_count", &n_group_used);
...
static DS4_MAYBE_UNUSED bool required_bool(const ds4_model *m, const char *key) {
    bool v = false;
...
static DS4_MAYBE_UNUSED uint64_t routed_expert_block_bytes(uint32_t type) {
    switch (type) {
    case DS4_TENSOR_Q4_0:    return sizeof(block_q4_0);
    ...
    case DS4_TENSOR_Q6_K:    return tensor_type(type)->block_bytes;
    case DS4_TENSOR_Q8_0:    return tensor_type(type)->block_bytes;
    case DS4_TENSOR_F16:     return sizeof(uint16_t) * QK_K;
    case DS4_TENSOR_F32:     return sizeof(float) * QK_K;
...
static DS4_MAYBE_UNUSED uint64_t routed_expert_row_bytes(const ds4_tensor *t) {
    ...
    if (!info || info->block_elems == 0 || (t->dim[0] % info->block_elems) != 0) {
        ds4_die("routed expert row is not quant-block aligned");
    }
    return (t->dim[0] / info->block_elems) * info->block_bytes;
}
...
/* Verify every tensor dimension used by the Qwen3-Coder specialized pipeline.
 * Quantized tensor type can vary across Q4_K_M-style GGUFs, so the validator is
 * strict about semantic shape and leaves quant dispatch to the kernels. */
static void weights_validate_layout(const ds4_weights *w) {
    ...
    tensor_expect_dims(w->token_embd,  2, DS4_N_EMBD, DS4_N_VOCAB, 0);
    tensor_expect_dims(w->output_norm, 1, DS4_N_EMBD, 0, 0);
    tensor_expect_dims(w->output,      2, DS4_N_EMBD, DS4_N_VOCAB, 0);

    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        const ds4_layer_weights *l = &w->layer[il];
        tensor_expect_dims(l->attn_norm,      1, DS4_N_EMBD, 0, 0);
        tensor_expect_dims(l->attn_q_a,       2, DS4_N_EMBD, q_dim, 0);
        tensor_expect_dims(l->attn_q_a_norm,  1, DS4_N_HEAD_DIM, 0, 0);
        tensor_expect_dims(l->attn_q_b,       2, DS4_N_EMBD, kv_dim, 0);
        tensor_expect_dims(l->attn_kv_a_norm, 1, DS4_N_HEAD_DIM, 0, 0);
        tensor_expect_dims(l->attn_kv,        2, DS4_N_EMBD, kv_dim, 0);
        tensor_expect_dims(l->attn_output_a,  2, q_dim, DS4_N_EMBD, 0);
        tensor_expect_dims(l->ffn_norm,       1, DS4_N_EMBD, 0, 0);
        tensor_expect_dims(l->ffn_gate_inp,   2, DS4_N_EMBD, DS4_N_EXPERT, 0);
...
static void eagle3_weights_validate_layout(const ds4_eagle3_weights *w) {
    const uint64_t target_features = (uint64_t)DS4_EAGLE3_N_EXTRACT * DS4_N_EMBD;
    const uint64_t attn_input = 2u * DS4_EAGLE3_N_EMBD;
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    const uint64_t kv_dim = (uint64_t)DS4_N_HEAD_KV * DS4_N_HEAD_DIM;

    tensor_expect_layout(w->fc,          DS4_TENSOR_F16, 2, target_features, DS4_EAGLE3_N_EMBD, 0);
    tensor_expect_layout(w->output,      DS4_TENSOR_F16, 2, DS4_EAGLE3_N_EMBD, DS4_EAGLE3_N_VOCAB, 0);
    tensor_expect_layout(w->output_norm, DS4_TENSOR_F32, 1, DS4_EAGLE3_N_EMBD, 0, 0);
    tensor_expect_layout(w->d2t,         DS4_TENSOR_I64, 1, DS4_EAGLE3_N_VOCAB, 0, 0);
    tensor_expect_layout(w->hidden_norm, DS4_TENSOR_F32, 1, DS4_EAGLE3_N_EMBD, 0, 0);
    tensor_expect_layout(w->attn_norm,   DS4_TENSOR_F32, 1, DS4_EAGLE3_N_EMBD, 0, 0);
    tensor_expect_layout(w->attn_q,      DS4_TENSOR_F16, 2, attn_input, q_dim, 0);
    tensor_expect_layout(w->attn_k,      DS4_TENSOR_F16, 2, attn_input, kv_dim, 0);
    tensor_expect_layout(w->attn_v,      DS4_TENSOR_F16, 2, attn_input, kv_dim, 0);
    tensor_expect_layout(w->attn_output, DS4_TENSOR_F16, 2, q_dim, DS4_EAGLE3_N_EMBD, 0);
    tensor_expect_layout(w->ffn_norm,    DS4_TENSOR_F32, 1, DS4_EAGLE3_N_EMBD, 0, 0);
    tensor_expect_layout(w->ffn_gate,    DS4_TENSOR_F16, 2, DS4_EAGLE3_N_EMBD, DS4_EAGLE3_N_FF, 0);
    tensor_expect_layout(w->ffn_up,      DS4_TENSOR_F16, 2, DS4_EAGLE3_N_EMBD, DS4_EAGLE3_N_FF, 0);
    tensor_expect_layout(w->ffn_down,    DS4_TENSOR_F16, 2, DS4_EAGLE3_N_FF, DS4_EAGLE3_N_EMBD, 0);
}
...
static void eagle3_config_validate(const ds4_model *m) {
    ds4_str arch = {0};
    if (!model_get_string(m, "general.architecture", &arch) || !ds4_streq(arch, "eagle3")) {
        fprintf(stderr, "ds4: --mtp requires a Qwen3-Coder EAGLE3 GGUF\n");
        exit(1);
    }
    config_expect_u32("eagle3.block_count",
                      required_u32(m, "eagle3.block_count"),
                      DS4_EAGLE3_N_LAYER);
    config_expect_u32("eagle3.embedding_length",
                      required_u32(m, "eagle3.embedding_length"),
                      DS4_EAGLE3_N_EMBD);
    config_expect_u32("eagle3.feed_forward_length",
                      required_u32(m, "eagle3.feed_forward_length"),
                      DS4_EAGLE3_N_FF);
    config_expect_u32("eagle3.target_hidden_size",
                      required_u32(m, "eagle3.target_hidden_size"),
                      DS4_N_EMBD);
    config_expect_u32("eagle3.attention.head_count",
                      required_u32(m, "eagle3.attention.head_count"),
                      DS4_N_HEAD);
    config_expect_u32("eagle3.attention.head_count_kv",
                      required_u32(m, "eagle3.attention.head_count_kv"),
                      DS4_N_HEAD_KV);
    config_expect_f32("eagle3.rope.freq_base",
                      required_f32(m, "eagle3.rope.freq_base"),
                      1000000.0f);

    ds4_array_ref layers = {0};
    if (!model_get_array(m, "eagle3.extract_layers", &layers) ||
        (layers.type != GGUF_VALUE_INT32 && layers.type != GGUF_VALUE_UINT32) ||
        layers.len != DS4_EAGLE3_N_EXTRACT) {
        ds4_die("eagle3.extract_layers must be the fixed Qwen3-Coder three-layer layout");
    }
    ds4_cursor c = cursor_at(m, layers.data_pos);
    for (uint32_t i = 0; i < DS4_EAGLE3_N_EXTRACT; i++) {
        uint32_t layer = 0;
        if (layers.type == GGUF_VALUE_UINT32) {
            if (!cursor_u32(&c, &layer)) ds4_die(c.error);
        } else {
            int32_t v = 0;
            if (!cursor_read(&c, &v, sizeof(v)) || v < 0) {
                ds4_die("eagle3.extract_layers contains an invalid layer");
            }
            layer = (uint32_t)v;
        }
        config_expect_u32("eagle3.extract_layers", layer, DS4_EAGLE3_EXTRACT_LAYERS[i]);
    }
}

static DS4_MAYBE_UNUSED void validate_compress_ratio_metadata(const ds4_model *m) {
...
static DS4_MAYBE_UNUSED void validate_swiglu_clamp_metadata(const ds4_model *m) {
...
static void config_expect_u32(const char *name, uint32_t got, uint32_t expected) {
    ...
    fprintf(stderr, "ds4: expected %s=%u for Qwen3-Coder-30B-A3B-Instruct, got %u\n",
            name, expected, got);
    exit(1);
}
...
static void config_expect_f32(const char *name, float got, float expected) {
    ...
    fprintf(stderr, "ds4: expected %s=%.9g for Qwen3-Coder-30B-A3B-Instruct, got %.9g\n",
            name, (double)expected, (double)got);
    exit(1);
}

static DS4_MAYBE_UNUSED void config_expect_bool(const char *name, bool got, bool expected) {
    if (got == expected) return;
    fprintf(stderr, "ds4: expected %s=%s for Qwen3-Coder-30B-A3B-Instruct, got %s\n",
...
/* Validate metadata values that affect Qwen3-Coder semantics: attention shape,
 * grouped KV heads, MoE routing, RoPE, and context length. */
static void config_validate_model(const ds4_model *m) {
    if (getenv("DS4_DUMP_METADATA_KEYS")) {
        for (uint64_t i = 0; i < m->n_kv; i++) {
    ...
    ds4_str arch = {0};
    ds4_str name = {0};
    if (!model_get_string(m, "general.architecture", &arch) || !ds4_streq(arch, "qwen3moe")) {
        fprintf(stderr, "ds4: expected general.architecture=qwen3moe for Qwen3-Coder\n");
        exit(1);
    }
    if (model_get_string(m, "general.name", &name) &&
        !ds4_strstr(name, "Qwen3-Coder-30B-A3B-Instruct") &&
        !ds4_strstr(name, "Qwen3 Coder 30B A3B Instruct")) {
        fprintf(stderr, "ds4: expected Qwen3-Coder-30B-A3B-Instruct GGUF, got %.*s\n",
                (int)name.len, name.ptr);
        exit(1);
    }

    const uint32_t n_layer = required_u32(m, "qwen3moe.block_count");
    const uint32_t n_embd = required_u32(m, "qwen3moe.embedding_length");
    ds4_array_ref token_arr = {0};
    if (!model_get_array(m, "tokenizer.ggml.tokens", &token_arr) ||
        token_arr.type != GGUF_VALUE_STRING ||
        token_arr.len > UINT32_MAX) {
        ds4_die("GGUF tokenizer token table is missing or invalid");
    }
    const uint32_t n_vocab = (uint32_t)token_arr.len;
    const uint32_t n_head = required_u32(m, "qwen3moe.attention.head_count");
    const uint32_t n_head_kv = required_u32(m, "qwen3moe.attention.head_count_kv");
    const uint32_t n_head_dim = required_u32(m, "qwen3moe.attention.key_length");
    const uint32_t n_value_dim = required_u32(m, "qwen3moe.attention.value_length");
    uint32_t n_rot = DS4_N_ROT;
    model_get_u32(m, "qwen3moe.rope.dimension_count", &n_rot);
    const uint32_t n_ff = required_u32(m, "qwen3moe.feed_forward_length");
    const uint32_t n_expert = required_u32(m, "qwen3moe.expert_count");
    const uint32_t n_expert_used = required_u32(m, "qwen3moe.expert_used_count");
    const uint32_t n_ff_exp = required_u32(m, "qwen3moe.expert_feed_forward_length");
    const uint64_t ctx_train = required_u64(m, "qwen3moe.context_length");
    ...
    config_expect_u32("feed_forward_length",         n_ff,           DS4_N_FF);
    ...
    if (ctx_train != UINT64_C(262144)) {
        fprintf(stderr, "ds4: expected qwen3moe.context_length=262144, got %" PRIu64 "\n", ctx_train);
        exit(1);
    }
    const float rope_freq_base = required_f32(m, "qwen3moe.rope.freq_base");
    config_expect_f32("rope.freq_base", rope_freq_base, 10000000.0f);
    const float rms_eps = required_f32(m, "qwen3moe.attention.layer_norm_rms_epsilon");
...
static void weights_bind(ds4_weights *w, const ds4_model *m) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        l->attn_q_a        = required_tensorf(m, "blk.%u.attn_q.weight", il);
        l->attn_q_a_norm   = required_tensorf(m, "blk.%u.attn_q_norm.weight", il);
        l->attn_q_b        = required_tensorf(m, "blk.%u.attn_k.weight", il);
        l->attn_kv_a_norm  = required_tensorf(m, "blk.%u.attn_k_norm.weight", il);
        l->attn_kv         = required_tensorf(m, "blk.%u.attn_v.weight", il);
...
static void eagle3_weights_bind(ds4_eagle3_weights *w, const ds4_model *m) {
    memset(w, 0, sizeof(*w));
    eagle3_config_validate(m);
    w->fc          = required_tensor(m, "fc.weight");
    w->output      = required_tensor(m, "output.weight");
    w->output_norm = required_tensor(m, "output_norm.weight");
    w->d2t         = required_tensor(m, "d2t");
    w->hidden_norm = required_tensor(m, "blk.0.hidden_norm.weight");
    w->attn_norm   = required_tensor(m, "blk.0.attn_norm.weight");
    w->attn_q      = required_tensor(m, "blk.0.attn_q.weight");
    w->attn_k      = required_tensor(m, "blk.0.attn_k.weight");
    w->attn_v      = required_tensor(m, "blk.0.attn_v.weight");
    w->attn_output = required_tensor(m, "blk.0.attn_output.weight");
    w->ffn_norm    = required_tensor(m, "blk.0.ffn_norm.weight");
    w->ffn_gate    = required_tensor(m, "blk.0.ffn_gate.weight");
    w->ffn_up      = required_tensor(m, "blk.0.ffn_up.weight");
    w->ffn_down    = required_tensor(m, "blk.0.ffn_down.weight");
    eagle3_weights_validate_layout(w);
}
...
static void dequant_q4_k_block(float *out, const block_q4_K *b) {
    const uint8_t *q = b->qs;
    const float d = f16_to_f32(b->d);
    const float min = f16_to_f32(b->dmin);
    int is = 0;
    for (int j = 0; j < QK_K; j += 64) {
        uint8_t sc, m;
        get_scale_min_k4_cpu(is + 0, b->scales, &sc, &m);
        const float d1 = d * (float)sc;
        const float m1 = min * (float)m;
        get_scale_min_k4_cpu(is + 1, b->scales, &sc, &m);
        const float d2 = d * (float)sc;
        const float m2 = min * (float)m;
        for (int l = 0; l < 32; l++) out[j + l] = d1 * (float)(q[l] & 0x0F) - m1;
...
static void dequant_q6_k_block(float *out, const block_q6_K *b) {
    const float d = f16_to_f32(b->d);
...
static void dequant_k_row(float *out, const uint8_t *row, uint64_t n, uint32_t type) {
    if ((n % QK_K) != 0) ds4_die("K-quant row is not QK_K aligned");
    for (uint64_t off = 0; off < n; off += QK_K) {
        if (type == DS4_TENSOR_Q4_K) {
            dequant_q4_k_block(out + off, (const block_q4_K *)row);
            row += sizeof(block_q4_K);
        } else if (type == DS4_TENSOR_Q6_K) {
            dequant_q6_k_block(out + off, (const block_q6_K *)row);
            row += sizeof(block_q6_K);
        } else {
            ds4_die("unsupported K-quant row type");
        }
    }
}
...
static void embed_token_any(const ds4_model *m, const ds4_weights *w, int token, float *out) {
    ...
    const uint64_t dim = te->dim[0];
    if (te->type == DS4_TENSOR_F32) {
        const float *row = (const float *)tensor_data(m, te) + (uint64_t)token * dim;
        memcpy(out, row, (size_t)dim * sizeof(out[0]));
    } else if (te->type == DS4_TENSOR_F16) {
        const uint16_t *row = (const uint16_t *)tensor_data(m, te) + (uint64_t)token * dim;
        for (uint64_t i = 0; i < dim; i++) out[i] = f16_to_f32(row[i]);
    } else if (te->type == DS4_TENSOR_Q4_K || te->type == DS4_TENSOR_Q6_K) {
        const uint64_t row_bytes = te->bytes / te->dim[1];
        const uint8_t *row = (const uint8_t *)tensor_data(m, te) + (uint64_t)token * row_bytes;
        dequant_k_row(out, row, dim, te->type);
    } else {
...
static float dot_q4_k_block(const block_q4_K *b, const float *x) {
    const uint8_t *q = b->qs;
    const float d = f16_to_f32(b->d);
    const float min = f16_to_f32(b->dmin);
    float acc = 0.0f;
    ...
    for (int j = 0; j < QK_K; j += 64) {
        ...
        const float d2 = d * (float)sc;
        const float m2 = min * (float)m;
        for (int l = 0; l < 32; l++) acc += (d1 * (float)(q[l] & 0x0F) - m1) * x[j + l];
        for (int l = 0; l < 32; l++) acc += (d2 * (float)(q[l] >> 4) - m2) * x[j + 32 + l];
        q += 32;
        is += 2;
    }
    return acc;
...
static float dot_q6_k_block(const block_q6_K *b, const float *x) {
    const float d = f16_to_f32(b->d);
    const uint8_t *ql = b->ql;
    const uint8_t *qh = b->qh;
    const int8_t *sc = b->scales;
    ...
    for (int n = 0; n < QK_K; n += 128) {
        for (int l = 0; l < 32; l++) {
            ...
            const int8_t q3 = (int8_t)((ql[l +  0] >>  4) | (((qh[l] >> 4) & 3) << 4)) - 32;
            const int8_t q4 = (int8_t)((ql[l + 32] >>  4) | (((qh[l] >> 6) & 3) << 4)) - 32;
            acc += d * (float)sc[is + 0] * (float)q1 * x[n + l +  0];
            acc += d * (float)sc[is + 2] * (float)q2 * x[n + l + 32];
            acc += d * (float)sc[is + 4] * (float)q3 * x[n + l + 64];
            acc += d * (float)sc[is + 6] * (float)q4 * x[n + l + 96];
        }
        ql += 64;
        qh += 32;
        sc += 8;
...
static void matvec_k_worker(void *vctx, uint64_t row0, uint64_t row1) {
    matvec_k_ctx *ctx = vctx;
    for (uint64_t row = row0; row < row1; row++) {
        const uint8_t *p = ctx->data + row * ctx->row_bytes;
        float acc = 0.0f;
        for (uint64_t off = 0; off < ctx->in_dim; off += QK_K) {
            if (ctx->type == DS4_TENSOR_Q4_K) {
                acc += dot_q4_k_block((const block_q4_K *)p, ctx->x + off);
                p += sizeof(block_q4_K);
            } else {
                acc += dot_q6_k_block((const block_q6_K *)p, ctx->x + off);
                p += sizeof(block_q6_K);
            }
        }
        ctx->out[row] = acc;
...
static void matvec_k_quant(float *out, const ds4_model *m, const ds4_tensor *w, const float *x) {
    if (w->ndim != 2 || (w->type != DS4_TENSOR_Q4_K && w->type != DS4_TENSOR_Q6_K)) {
        ds4_die("expected a 2D Q4_K/Q6_K tensor");
    }
    if ((w->dim[0] % QK_K) != 0) ds4_die("K-quant row is not QK_K aligned");
    matvec_k_ctx ctx = {
        .out = out,
        .data = tensor_data(m, w),
        .x = x,
        .in_dim = w->dim[0],
        .row_bytes = w->bytes / w->dim[1],
        .type = w->type,
    };
    ds4_parallel_for(w->dim[1], matvec_k_worker, &ctx);
}

/* Dispatch for dense tensors used by Qwen projections. */
static void matvec_any(float *out, const ds4_model *m, const ds4_tensor *w, const float *x) {
    switch (w->type) {
    ...
    case 12:
    case 14: matvec_k_quant(out, m, w, x); break;
    default:
...
static void matvec_k_expert_worker(void *vctx, uint64_t row0, uint64_t row1) {
    matvec_k_expert_ctx *ctx = vctx;
    for (uint64_t row = row0; row < row1; row++) {
        const uint8_t *p = ctx->data + row * ctx->row_bytes;
        float acc = 0.0f;
        for (uint64_t off = 0; off < ctx->in_dim; off += QK_K) {
...
static void matvec_k_expert(float *out, const ds4_model *m, const ds4_tensor *w,
                            const float *x, uint32_t expert) {
    if (w->ndim != 3 || (w->type != DS4_TENSOR_Q4_K && w->type != DS4_TENSOR_Q6_K)) {
        ds4_die("expected a 3D Q4_K/Q6_K expert tensor");
    }
    if (expert >= w->dim[2]) ds4_die("expert index is outside routed tensor");
    if ((w->dim[0] % QK_K) != 0) ds4_die("expert tensor row is not QK_K aligned");

    const uint64_t row_bytes = w->bytes / (w->dim[1] * w->dim[2]);
    const uint8_t *base = (const uint8_t *)tensor_data(m, w) +
        (uint64_t)expert * w->dim[1] * row_bytes;
    matvec_k_expert_ctx ctx = {
        .out = out,
        .data = base,
        .x = x,
        .in_dim = w->dim[0],
    ...
    ds4_parallel_for(w->dim[1], matvec_k_expert_worker, &ctx);
}

static void qwen_apply_head_rms_weight(float *x, uint32_t n_head, const float *weight) {
    for (uint32_t h = 0; h < n_head; h++) {
        float *head = x + (uint64_t)h * DS4_N_HEAD_DIM;
        double ss = 0.0;
        for (uint32_t i = 0; i < DS4_N_HEAD_DIM; i++) ss += (double)head[i] * head[i];
        const float scale = 1.0f / sqrtf((float)(ss / (double)DS4_N_HEAD_DIM) + DS4_RMS_EPS);
        for (uint32_t i = 0; i < DS4_N_HEAD_DIM; i++) head[i] = head[i] * scale * weight[i];
    }
}

static void qwen_rope_inplace(float *x, uint32_t n_head, uint32_t pos) {
    const uint32_t half = DS4_N_HEAD_DIM / 2;
    for (uint32_t h = 0; h < n_head; h++) {
        float *head = x + (uint64_t)h * DS4_N_HEAD_DIM;
        for (uint32_t i = 0; i < half; i++) {
            const float theta = (float)pos *
                powf(DS4_ROPE_FREQ_BASE, -((float)(2u * i) / (float)DS4_N_HEAD_DIM));
...
static void qwen_router_topk(
        int                  selected[DS4_N_EXPERT_USED],
        float                weight[DS4_N_EXPERT_USED],
        const ds4_model     *model,
        const ds4_layer_weights *layer,
        const float         *x,
        float               *logits_out) {
    float logits[DS4_N_EXPERT];
    matvec_f32(logits, model, layer->ffn_gate_inp, x);
    ...
    for (int i = 1; i < DS4_N_EXPERT_USED; i++) {
        if (logits[selected[i]] > maxv) maxv = logits[selected[i]];
    }
    double sum = 0.0;
    for (int i = 0; i < DS4_N_EXPERT_USED; i++) {
        const double v = exp((double)logits[selected[i]] - (double)maxv);
        weight[i] = (float)v;
        sum += v;
    }
    if (sum <= 0.0) sum = 1.0;
...
static bool qwen_cpu_debug_enabled(void);
static bool qwen_cpu_debug_wants(const char *name, uint32_t il, uint32_t pos);
static void qwen_cpu_debug_dump_tensor(
        const char *name,
        const float *data,
        uint64_t n_f32,
        uint32_t il,
...
static void qwen_moe_one(float *out, const ds4_model *model,
                         ...
                         uint32_t il, uint32_t pos, int token) {
    int selected[DS4_N_EXPERT_USED];
    float expert_weight[DS4_N_EXPERT_USED];
    float router_logits[DS4_N_EXPERT];
    ...
    float *dbg_gate = qwen_cpu_debug_enabled() ?
        xmalloc((size_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP * sizeof(float)) : NULL;
    float *dbg_up = qwen_cpu_debug_enabled() ?
        xmalloc((size_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP * sizeof(float)) : NULL;
    float *dbg_mid = qwen_cpu_debug_enabled() ?
        xmalloc((size_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP * sizeof(float)) : NULL;
    float *dbg_down = qwen_cpu_debug_enabled() ?
        xmalloc((size_t)DS4_N_EXPERT_USED * DS4_N_EMBD * sizeof(float)) : NULL;

    memset(out, 0, (size_t)DS4_N_EMBD * sizeof(out[0]));
    qwen_router_topk(selected, expert_weight, model, layer, x, router_logits);
    qwen_cpu_debug_dump_tensor("router_logits", router_logits, DS4_N_EXPERT, il, pos);
    qwen_cpu_debug_dump_tensor("router_weights", expert_weight, DS4_N_EXPERT_USED, il, pos);
    if (qwen_cpu_debug_wants("router_selected", il, pos)) {
        float selected_f[DS4_N_EXPERT_USED];
        for (uint32_t i = 0; i < DS4_N_EXPERT_USED; i++) selected_f[i] = (float)selected[i];
        qwen_cpu_debug_dump_tensor("router_selected", selected_f, DS4_N_EXPERT_USED, il, pos);
    }
    ...
    for (int i = 0; i < DS4_N_EXPERT_USED; i++) {
        ...
        matvec_k_expert(up, model, layer->ffn_up_exps, x, expert);
        for (uint32_t j = 0; j < DS4_N_FF_EXP; j++) {
            mid[j] = silu(gate[j]) * up[j] * expert_weight[i];
        }
        matvec_k_expert(down, model, layer->ffn_down_exps, mid, expert);
        if (dbg_gate) memcpy(dbg_gate + (uint64_t)i * DS4_N_FF_EXP, gate,
                             (size_t)DS4_N_FF_EXP * sizeof(float));
        if (dbg_up) memcpy(dbg_up + (uint64_t)i * DS4_N_FF_EXP, up,
                           (size_t)DS4_N_FF_EXP * sizeof(float));
        if (dbg_mid) memcpy(dbg_mid + (uint64_t)i * DS4_N_FF_EXP, mid,
                            (size_t)DS4_N_FF_EXP * sizeof(float));
        if (dbg_down) memcpy(dbg_down + (uint64_t)i * DS4_N_EMBD, down,
                             (size_t)DS4_N_EMBD * sizeof(float));
        for (uint32_t j = 0; j < DS4_N_EMBD; j++) out[j] += down[j];
    }
    if (dbg_gate) qwen_cpu_debug_dump_tensor("moe_gate", dbg_gate,
                                             (uint64_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP, il, pos);
    if (dbg_up) qwen_cpu_debug_dump_tensor("moe_up", dbg_up,
                                           (uint64_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP, il, pos);
    if (dbg_mid) qwen_cpu_debug_dump_tensor("moe_mid", dbg_mid,
                                            (uint64_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP, il, pos);
    if (dbg_down) qwen_cpu_debug_dump_tensor("moe_down", dbg_down,
 ...
 * When the HC tensors are absent (e.g. Qwen3-Coder), the logic degenerates to
 * a plain residual copy because DS4_N_HC == 1 and there is no mixing to do. */
static void hc_pre_from_state_one_scratch(
    ...
    if (!fn) {
        if (n_hc != 1) {
            ds4_die("hc_pre_from_state_one_scratch: missing HC tensors with n_hc != 1");
        }
...
static uint32_t ds4_default_raw_cap(uint32_t ctx_size) {
    /* Qwen3-Coder has no sliding window and no KV compression; the raw cache
     * must hold the full context so decode attends to every previous token. */
    return ctx_size ? ctx_size : 1u;
}
...
static void kv_cache_init(ds4_kv_cache *cache, uint32_t ctx_size, uint32_t raw_cap) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        cache->layer[il].raw_k = xmalloc_zeroed((size_t)raw_cap * kv_dim, sizeof(float));
        cache->layer[il].raw_v = xmalloc_zeroed((size_t)raw_cap * kv_dim, sizeof(float));
...
static void kv_cache_free(ds4_kv_cache *cache) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        free(cache->layer[il].raw_k);
        free(cache->layer[il].raw_v);
...
static bool qwen_cpu_debug_enabled(void) {
    const char *prefix = getenv("DS4_CPU_QWEN_DUMP_PREFIX");
    return prefix && prefix[0];
}

static bool qwen_cpu_debug_wants(const char *name, uint32_t il, uint32_t pos) {
    if (!qwen_cpu_debug_enabled()) return false;
    const char *name_env = getenv("DS4_CPU_QWEN_DUMP_NAME");
    if (name_env && name_env[0] && strstr(name_env, name) == NULL) return false;
    const char *layer_env = getenv("DS4_CPU_QWEN_DUMP_LAYER");
    if (layer_env && layer_env[0] && strcmp(layer_env, "all") != 0 &&
        (uint32_t)strtoul(layer_env, NULL, 10) != il) return false;
    const char *pos_env = getenv("DS4_CPU_QWEN_DUMP_POS");
...
static void qwen_cpu_debug_dump_tensor(
        ...
        uint32_t il,
        uint32_t pos) {
    if (!data || n_f32 == 0 || !qwen_cpu_debug_wants(name, il, pos)) return;
    const char *prefix = getenv("DS4_CPU_QWEN_DUMP_PREFIX");
    char path[1024];
    snprintf(path, sizeof(path), "%s_%s-%u_pos%u.bin", prefix, name, il, pos);
    if (write_f32_binary_file(path, data, n_f32)) {
        fprintf(stderr, "ds4: CPU dumped %s layer %u pos %u to %s\n", name, il, pos, path);
    }
}
...
static void forward_token_qwen_cpu(
        float             * logits,
        const ds4_model   * model,
        const ds4_weights * weights,
        ds4_kv_cache      * cache,
    ...
    float *q = xmalloc((size_t)q_dim * sizeof(q[0]));
    float *k = xmalloc((size_t)kv_dim * sizeof(k[0]));
    float *v = xmalloc((size_t)kv_dim * sizeof(v[0]));
    float *heads = xmalloc((size_t)q_dim * sizeof(heads[0]));
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        matvec_any(k, model, layer->attn_q_b, norm);
        matvec_any(v, model, layer->attn_kv, norm);
        qwen_apply_head_rms_weight(q, DS4_N_HEAD, q_norm);
        qwen_apply_head_rms_weight(k, DS4_N_HEAD_KV, k_norm);
        qwen_rope_inplace(q, DS4_N_HEAD, pos);
        qwen_rope_inplace(k, DS4_N_HEAD_KV, pos);

        /* Store K and V in the layer cache. */
        if (lc->n_raw < lc->cap_raw) {
            float *dk = lc->raw_k + (uint64_t)lc->n_raw * kv_dim;
            float *dv = lc->raw_v + (uint64_t)lc->n_raw * kv_dim;
            memcpy(dk, k, (size_t)kv_dim * sizeof(k[0]));
            memcpy(dv, v, (size_t)kv_dim * sizeof(v[0]));
            lc->n_raw++;
        } else {
        ...
        /* Compute attention over all cached tokens. */
        for (uint32_t h = 0; h < DS4_N_HEAD; h++) {
        ...
        qwen_cpu_debug_dump_tensor("attn_out", attn_out, d, il, pos);
        ...
        qwen_cpu_debug_dump_tensor("ffn_norm", norm, d, il, pos);
        qwen_moe_one(ffn_out, model, layer, norm, il, pos, token);
        qwen_cpu_debug_dump_tensor("ffn_out", ffn_out, d, il, pos);
        cpu_directional_steering_project_rows_multi(
        ...
        qwen_cpu_debug_dump_tensor("resid", x, d, il, pos);
...
static DS4_MAYBE_UNUSED void prefill_layer_major_cpu(
        float             * logits,
        const ds4_model   * model,
        const ds4_weights * weights,
        ds4_kv_cache      * cache,
...
typedef struct {
    ...
    ds4_gpu_tensor *kv;
    ds4_gpu_tensor *rope_cos;
    ds4_gpu_tensor *rope_sin;
    ...
    ds4_gpu_tensor *layer_v_cache[DS4_N_LAYER];
    ...
    ds4_gpu_tensor *router_cache_f16;
    ...
    uint32_t mtp_n_raw;
    /* Qwen3-Coder EAGLE3 drafter.  The public option remains --mtp for API
     * compatibility, but the only accepted support model is this fixed EAGLE3
     * layout. */
    const ds4_model *eagle3_model;
    const ds4_eagle3_weights *eagle3_weights;
    ds4_gpu_tensor *eagle3_target_features;
    ds4_gpu_tensor *eagle3_target_g;
    ds4_gpu_tensor *eagle3_pending_g;
    ds4_gpu_tensor *eagle3_batch_features;
    ds4_gpu_tensor *eagle3_batch_g;
    ds4_gpu_tensor *eagle3_embed_norm;
    ds4_gpu_tensor *eagle3_hidden_norm;
    ds4_gpu_tensor *eagle3_input;
    ds4_gpu_tensor *eagle3_q;
    ds4_gpu_tensor *eagle3_k;
    ds4_gpu_tensor *eagle3_v;
    ds4_gpu_tensor *eagle3_heads;
    ds4_gpu_tensor *eagle3_attn_out;
    ds4_gpu_tensor *eagle3_after_attn;
    ds4_gpu_tensor *eagle3_ffn_norm;
    ds4_gpu_tensor *eagle3_gate;
    ds4_gpu_tensor *eagle3_up;
    ds4_gpu_tensor *eagle3_mid;
    ds4_gpu_tensor *eagle3_down;
    ds4_gpu_tensor *eagle3_output_norm;
    ds4_gpu_tensor *eagle3_logits;
    ds4_gpu_tensor *eagle3_k_cache;
    ds4_gpu_tensor *eagle3_v_cache;
    bool eagle3_capture_batch;
    bool eagle3_pending_valid;
...
static void metal_graph_free(ds4_gpu_graph *g) {
    for (int i = 0; i < DS4_MAX_STEERING_VECTORS; i++) {
        ds4_gpu_tensor_free(g->steering_dirs[i]);
    }
    ...
    ds4_gpu_tensor_free(g->eagle3_v_cache);
    ds4_gpu_tensor_free(g->eagle3_k_cache);
    ds4_gpu_tensor_free(g->eagle3_logits);
    ds4_gpu_tensor_free(g->eagle3_output_norm);
    ds4_gpu_tensor_free(g->eagle3_down);
    ds4_gpu_tensor_free(g->eagle3_mid);
    ds4_gpu_tensor_free(g->eagle3_up);
    ds4_gpu_tensor_free(g->eagle3_gate);
    ds4_gpu_tensor_free(g->eagle3_ffn_norm);
    ds4_gpu_tensor_free(g->eagle3_after_attn);
    ds4_gpu_tensor_free(g->eagle3_attn_out);
    ds4_gpu_tensor_free(g->eagle3_heads);
    ds4_gpu_tensor_free(g->eagle3_v);
    ds4_gpu_tensor_free(g->eagle3_k);
    ds4_gpu_tensor_free(g->eagle3_q);
    ds4_gpu_tensor_free(g->eagle3_input);
    ds4_gpu_tensor_free(g->eagle3_hidden_norm);
    ds4_gpu_tensor_free(g->eagle3_embed_norm);
    ds4_gpu_tensor_free(g->eagle3_batch_g);
    ds4_gpu_tensor_free(g->eagle3_batch_features);
    ds4_gpu_tensor_free(g->eagle3_pending_g);
    ds4_gpu_tensor_free(g->eagle3_target_g);
    ds4_gpu_tensor_free(g->eagle3_target_features);
    ...
    ds4_gpu_tensor_free(g->router_cache_f16);
    ...
    ds4_gpu_tensor_free(g->rope_sin);
    ds4_gpu_tensor_free(g->rope_cos);
...
static bool metal_graph_load_directional_steering_single(
        ds4_gpu_graph *g,
        int            idx,
        const char     *path,
        float          attn_scale,
    ...
    if (ok) {
        g->steering_dirs[idx] = ds4_gpu_tensor_alloc(n * sizeof(dirs[0]));
        ok = g->steering_dirs[idx] != NULL &&
             ds4_gpu_tensor_write(g->steering_dirs[idx], 0, dirs, n * sizeof(dirs[0])) != 0;
    }
    free(dirs);
    ...
    g->steering_attn_scales[idx] = attn_scale;
    g->steering_ffn_scales[idx] = ffn_scale;
...
static bool metal_graph_load_directional_steering(
        ...
        int            n_steering,
        const char    **files,
        const float   * attn_scales,
        const float   * ffn_scales) {
    g->n_steering_vectors = 0;
    for (int i = 0; i < n_steering; i++) {
        if (!metal_graph_load_directional_steering_single(g, i, files[i],
                                                            attn_scales[i], ffn_scales[i])) {
            return false;
        }
        g->n_steering_vectors++;
    }
    /* Legacy single-vector support */
    if (n_steering == 0) {
        g->directional_steering_dirs = NULL;
...
static bool metal_graph_directional_steering_attn_enabled(const ds4_gpu_graph *g) {
    return g && g->n_steering_vectors > 0;
}
...
static bool metal_graph_apply_directional_steering_single(
        ds4_gpu_graph  *g,
        ds4_gpu_tensor *x,
        uint32_t          il,
        uint32_t          rows,
...
static bool qwen_graph_alloc(
    ...
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    const uint64_t kv_dim = (uint64_t)DS4_N_HEAD_KV * DS4_N_HEAD_DIM;
    const uint64_t routed_mid_dim = weights->layer[0].ffn_gate_exps->dim[1];
    const uint64_t vocab_dim = weights->output->dim[1];

    if (ctx_size == 0) ctx_size = 1;
    if (prefill_cap == 0) prefill_cap = 1;
    g->raw_cap = ctx_size;
    g->raw_window = ctx_size;
    g->prefill_cap = prefill_cap;

    g->cur_hc = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EMBD * sizeof(float));
    g->after_ffn_hc = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EMBD * sizeof(float));
    g->attn_norm = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EMBD * sizeof(float));
    g->q = ds4_gpu_tensor_alloc_private(q_dim * sizeof(float));
    g->kv_raw = ds4_gpu_tensor_alloc_private(kv_dim * sizeof(float));
    g->kv = ds4_gpu_tensor_alloc_private(kv_dim * sizeof(float));
    g->rope_cos = ds4_gpu_tensor_alloc((uint64_t)ctx_size * (DS4_N_HEAD_DIM / 2u) * sizeof(float));
    g->rope_sin = ds4_gpu_tensor_alloc((uint64_t)ctx_size * (DS4_N_HEAD_DIM / 2u) * sizeof(float));
    g->heads = ds4_gpu_tensor_alloc_private(q_dim * sizeof(float));
    g->attn_out = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EMBD * sizeof(float));
    g->ffn_norm = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EMBD * sizeof(float));
    g->router_logits = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT * sizeof(float));
    g->router_probs = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT * sizeof(float));
    g->router_selected = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT_USED * sizeof(int32_t));
    g->router_weights = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT_USED * sizeof(float));
    g->routed_gate = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT_USED * routed_mid_dim * sizeof(float));
    g->routed_up = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT_USED * routed_mid_dim * sizeof(float));
    g->routed_mid = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT_USED * routed_mid_dim * sizeof(float));
    g->routed_down = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EXPERT_USED * DS4_N_EMBD * sizeof(float));
    g->routed_out = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EMBD * sizeof(float));
    g->output_pre = ds4_gpu_tensor_alloc_private(vocab_dim * sizeof(float));
    g->output_norm = ds4_gpu_tensor_alloc_private((uint64_t)DS4_N_EMBD * sizeof(float));
    ...
    const uint64_t router_layer_bytes_f16 = router_layer_elems * sizeof(uint16_t);
    ...
    g->router_cache_f16 = ds4_gpu_tensor_alloc_private(router_cache_elems * sizeof(uint16_t));
    bool router_cache_ok = g->router_cache != NULL && g->router_cache_f16 != NULL;
    float *router_cache = router_cache_ok
        ? xmalloc((size_t)router_cache_elems * sizeof(router_cache[0]))
        : NULL;
    uint16_t *router_cache_f16 = router_cache_ok
        ? xmalloc((size_t)router_cache_elems * sizeof(router_cache_f16[0]))
        : NULL;
    for (uint32_t il = 0; router_cache_ok && il < DS4_N_LAYER; il++) {
        ...
        uint16_t *dst_f16 = router_cache_f16 + (uint64_t)il * router_layer_elems;
        memcpy(dst, src, (size_t)router_layer_bytes);
        for (uint64_t i = 0; i < router_layer_elems; i++) {
            dst_f16[i] = f32_to_f16(src[i]);
        }
    }
    if (router_cache_ok) {
    ...
    if (router_cache_ok) {
        router_cache_ok = ds4_gpu_tensor_write(
                ...
                0,
                router_cache_f16,
                (uint64_t)DS4_N_LAYER * router_layer_bytes_f16) != 0;
    }
    free(router_cache);
    free(router_cache_f16);
    ...
    g->batch_attn_norm = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EMBD * sizeof(float));
    g->batch_q = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * q_dim * sizeof(float));
    g->batch_kv_raw = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * kv_dim * sizeof(float));
    g->batch_kv = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * kv_dim * sizeof(float));
    g->batch_heads = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * q_dim * sizeof(float));
    g->batch_attn_out = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EMBD * sizeof(float));
    g->batch_after_attn_hc = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EMBD * sizeof(float));
    g->batch_ffn_norm = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EMBD * sizeof(float));
    g->batch_router_logits = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EXPERT * sizeof(float));
    g->batch_router_probs = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EXPERT * sizeof(float));
    g->batch_router_selected = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EXPERT_USED * sizeof(int32_t));
    g->batch_router_weights = ds4_gpu_tensor_alloc((uint64_t)prefill_cap * DS4_N_EXPERT_USED * sizeof(float));
    ...
    const bool ok = cache_ok && router_cache_ok &&
                    ...
                    g->output_pre && g->output_norm && g->logits &&
                    g->top_id &&
                    ...
                    g->batch_after_attn_hc && g->batch_ffn_norm &&
    ...
    if (ok) {
        enum { HALF_DIM = DS4_N_HEAD_DIM / 2 };
        const uint64_t elems = (uint64_t)ctx_size * HALF_DIM;
        float *c = xmalloc((size_t)elems * sizeof(c[0]));
        float *s = xmalloc((size_t)elems * sizeof(s[0]));
        for (uint32_t pos = 0; pos < ctx_size; pos++) {
            for (uint32_t i = 0; i < HALF_DIM; i++) {
                const float theta = (float)pos *
                    powf(DS4_ROPE_FREQ_BASE, -2.0f * (float)i / (float)DS4_N_HEAD_DIM);
                c[(uint64_t)pos * HALF_DIM + i] = cosf(theta);
                s[(uint64_t)pos * HALF_DIM + i] = sinf(theta);
            }
        }
        const uint64_t bytes = elems * sizeof(float);
        const bool wrote = ds4_gpu_tensor_write(g->rope_cos, 0, c, bytes) != 0 &&
                           ds4_gpu_tensor_write(g->rope_sin, 0, s, bytes) != 0;
        free(s);
        free(c);
        return wrote;
    }
...
static bool eagle3_graph_alloc(ds4_gpu_graph *g, uint32_t ctx_size, uint32_t prefill_cap) {
    const uint64_t target_features = (uint64_t)DS4_EAGLE3_N_EXTRACT * DS4_N_EMBD;
    const uint64_t attn_input = 2u * DS4_EAGLE3_N_EMBD;
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    const uint64_t kv_dim = (uint64_t)DS4_N_HEAD_KV * DS4_N_HEAD_DIM;
    ...
    g->spec_logits = ds4_gpu_tensor_alloc(
            (uint64_t)DS4_MAX_DRAFT_TOKENS * DS4_N_VOCAB * sizeof(float));
    g->comp_selected = ds4_gpu_tensor_alloc(
            (uint64_t)DS4_MAX_DRAFT_TOKENS * sizeof(int32_t));
    g->eagle3_target_features = ds4_gpu_tensor_alloc_private(target_features * sizeof(float));
    g->eagle3_target_g = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_pending_g = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_batch_features = ds4_gpu_tensor_alloc_private(
            (uint64_t)prefill_cap * target_features * sizeof(float));
    g->eagle3_batch_g = ds4_gpu_tensor_alloc_private(
            (uint64_t)prefill_cap * DS4_EAGLE3_N_EMBD * sizeof(float));
    g->mtp_embed = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_embed_norm = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_hidden_norm = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_input = ds4_gpu_tensor_alloc_private(attn_input * sizeof(float));
    g->eagle3_q = ds4_gpu_tensor_alloc_private(q_dim * sizeof(float));
    g->eagle3_k = ds4_gpu_tensor_alloc_private(kv_dim * sizeof(float));
    g->eagle3_v = ds4_gpu_tensor_alloc_private(kv_dim * sizeof(float));
    g->eagle3_heads = ds4_gpu_tensor_alloc_private(q_dim * sizeof(float));
    g->eagle3_attn_out = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_after_attn = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_ffn_norm = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_gate = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_FF * sizeof(float));
    g->eagle3_up = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_FF * sizeof(float));
    g->eagle3_mid = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_FF * sizeof(float));
    g->eagle3_down = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->mtp_state_hc = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->mtp_next_hc = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_output_norm = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float));
    g->eagle3_logits = ds4_gpu_tensor_alloc_private((uint64_t)DS4_EAGLE3_N_VOCAB * sizeof(float));
    g->eagle3_k_cache = ds4_gpu_tensor_alloc_private((uint64_t)ctx_size * kv_dim * sizeof(uint16_t));
    g->eagle3_v_cache = ds4_gpu_tensor_alloc_private((uint64_t)ctx_size * kv_dim * sizeof(uint16_t));

    return g->spec_logits && g->comp_selected &&
           g->eagle3_target_features && g->eagle3_target_g && g->eagle3_pending_g &&
           g->eagle3_batch_features && g->eagle3_batch_g &&
           g->mtp_embed && g->eagle3_embed_norm && g->eagle3_hidden_norm &&
           g->eagle3_input && g->eagle3_q && g->eagle3_k && g->eagle3_v &&
           g->eagle3_heads && g->eagle3_attn_out && g->eagle3_after_attn &&
           g->eagle3_ffn_norm && g->eagle3_gate && g->eagle3_up &&
           g->eagle3_mid && g->eagle3_down && g->mtp_state_hc &&
           g->mtp_next_hc && g->eagle3_output_norm && g->eagle3_logits &&
           g->eagle3_k_cache && g->eagle3_v_cache;
}
...
static bool metal_graph_alloc_raw_cap(
    ...
    (void)layer;
    const uint32_t cap = ctx_size ? ctx_size : raw_cap;
    bool ok = qwen_graph_alloc(g, model, weights, cap, prefill_cap);
    if (ok && enable_mtp) ok = eagle3_graph_alloc(g, cap, prefill_cap);
    if (!ok) metal_graph_free(g);
    return ok;
...
static bool metal_graph_encode_output_head_batch(
    ...
    bool ok = n_tokens <= DS4_MAX_DRAFT_TOKENS;
    ...
    if (ok) ok = metal_graph_matmul_plain_tensor(
            ...
            DS4_N_EMBD, vocab_dim, output_norm, n_tokens);
...
static bool metal_graph_matmul_plain_tiny_rows(
        ds4_gpu_tensor       *out,
        const ds4_model        *model,
        const ds4_tensor       *w,
        uint64_t                in_dim,
        ...
        const ds4_gpu_tensor *x,
        uint64_t                n_tok) {
    bool ok = n_tok > 1 && n_tok <= 4;
    const uint64_t in_row_bytes = in_dim * sizeof(float);
    const uint64_t out_row_bytes = out_dim * sizeof(float);
    for (uint64_t row = 0; ok && row < n_tok; row++) {
        ds4_gpu_tensor *x_row = ds4_gpu_tensor_view(
                x, row * in_row_bytes, in_row_bytes);
        ds4_gpu_tensor *out_row = ds4_gpu_tensor_view(
                out, row * out_row_bytes, out_row_bytes);
        ok = x_row && out_row;
        if (ok && w->type == DS4_TENSOR_F32) {
            ok = ds4_gpu_matmul_f32_tensor(
                    out_row, model->map, model->size, w->abs_offset,
                    in_dim, out_dim, x_row, 1) != 0;
        } else if (ok) {
            ok = ds4_gpu_matmul_k_tensor(
                    out_row, model->map, model->size, w->abs_offset, w->type,
                    in_dim, out_dim, x_row) != 0;
        }
...
static bool metal_graph_matmul_plain_tensor(
    ...
    if (w->type == DS4_TENSOR_F32) {
        if (n_tok > 1 && n_tok <= 4) {
            return metal_graph_matmul_plain_tiny_rows(
    ...
    if (w->type == DS4_TENSOR_Q2_K ||
        w->type == DS4_TENSOR_Q4_K ||
        w->type == DS4_TENSOR_Q6_K) {
        if (n_tok == 1) {
            return ds4_gpu_matmul_k_tensor(out, model->map, model->size,
                                           w->abs_offset, w->type,
                                           in_dim, out_dim, x) != 0;
        }
        if (n_tok <= 4) {
            return ds4_gpu_matmul_k_batch_tensor(out, model->map, model->size,
                                                 w->abs_offset, w->type,
...
static int eagle3_extract_slot(uint32_t il) {
    for (uint32_t i = 0; i < DS4_EAGLE3_N_EXTRACT; i++) {
        if (DS4_EAGLE3_EXTRACT_LAYERS[i] == il) return (int)i;
    }
    return -1;
}

static bool metal_graph_eagle3_capture_token_input(ds4_gpu_graph *g, uint32_t il) {
    if (!g || !g->eagle3_weights) return true;
    const int slot = eagle3_extract_slot(il);
    if (slot < 0) return true;
    return ds4_gpu_tensor_copy(
            g->eagle3_target_features,
...
static bool metal_graph_eagle3_capture_batch_input(
        ds4_gpu_graph *g,
        uint32_t il,
        uint32_t n_tokens) {
    if (!g || !g->eagle3_weights) return true;
    ...
    const uint64_t feature_row_bytes =
        (uint64_t)DS4_EAGLE3_N_EXTRACT * DS4_N_EMBD * sizeof(float);
    for (uint32_t row = 0; row < n_tokens; row++) {
        if (ds4_gpu_tensor_copy(
                g->eagle3_batch_features,
                (uint64_t)row * feature_row_bytes + (uint64_t)slot * row_bytes,
                g->batch_cur_hc,
...
static bool metal_graph_eagle3_finish_token_features(ds4_gpu_graph *g) {
    if (!g || !g->eagle3_weights || !g->eagle3_model) return true;
    const uint64_t target_features = (uint64_t)DS4_EAGLE3_N_EXTRACT * DS4_N_EMBD;
    bool ok = ds4_gpu_matmul_f16_tensor(
            g->eagle3_target_g,
            g->eagle3_model->map,
            g->eagle3_model->size,
            g->eagle3_weights->fc->abs_offset,
            target_features,
            DS4_EAGLE3_N_EMBD,
            g->eagle3_target_features,
            1) != 0;
    if (ok) {
        ok = ds4_gpu_tensor_copy(
                ...
                g->eagle3_target_g,
                0,
                (uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float)) != 0;
    }
    if (ok) g->eagle3_pending_valid = true;
    return ok;
}

static bool metal_graph_eagle3_finish_batch_features(
        ds4_gpu_graph *g,
        uint32_t n_tokens) {
    if (!g || !g->eagle3_weights || !g->eagle3_model) return true;
    ...
    return ds4_gpu_matmul_f16_tensor(
            ...
            g->eagle3_model->size,
            g->eagle3_weights->fc->abs_offset,
            (uint64_t)DS4_EAGLE3_N_EXTRACT * DS4_N_EMBD,
            DS4_EAGLE3_N_EMBD,
            g->eagle3_batch_features,
            n_tokens) != 0;
}

static bool metal_graph_encode_eagle3_decode(
        ds4_gpu_graph *g,
        const ds4_model *base_model,
        const ds4_weights *base_weights,
        const ds4_gpu_tensor *g_input,
        ds4_gpu_tensor *out_state,
        int token,
        uint32_t pos,
        bool need_top) {
    if (!g || !g->eagle3_model || !g->eagle3_weights || !g_input || !out_state ||
        token < 0 || (uint64_t)token >= base_weights->token_embd->dim[1] ||
        pos >= g->raw_cap) {
        return false;
    }
    const ds4_model *model = g->eagle3_model;
    const ds4_eagle3_weights *w = g->eagle3_weights;
    const ds4_tensor *te = base_weights->token_embd;
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    const uint64_t kv_dim = (uint64_t)DS4_N_HEAD_KV * DS4_N_HEAD_DIM;
    const uint64_t embd_bytes = (uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float);
    const uint64_t row_bytes = te->bytes / te->dim[1];

    bool ok = ds4_gpu_dequant_k_row_tensor(
            g->mtp_embed,
            base_model->map,
            base_model->size,
            te->abs_offset + (uint64_t)token * row_bytes,
            te->type,
            DS4_EAGLE3_N_EMBD) != 0;
    if (ok) ok = ds4_gpu_rms_norm_weight_tensor(
            g->eagle3_embed_norm, g->mtp_embed,
            model->map, model->size, w->attn_norm->abs_offset,
            DS4_EAGLE3_N_EMBD, DS4_RMS_EPS) != 0;
    if (ok) ok = ds4_gpu_rms_norm_weight_tensor(
            g->eagle3_hidden_norm, g_input,
            model->map, model->size, w->hidden_norm->abs_offset,
            DS4_EAGLE3_N_EMBD, DS4_RMS_EPS) != 0;
    if (ok) ok = ds4_gpu_tensor_copy(g->eagle3_input, 0,
                                     g->eagle3_embed_norm, 0, embd_bytes) != 0;
    if (ok) ok = ds4_gpu_tensor_copy(g->eagle3_input, embd_bytes,
                                     g->eagle3_hidden_norm, 0, embd_bytes) != 0;
    if (ok) ok = ds4_gpu_matmul_f16_tensor(
            g->eagle3_q, model->map, model->size, w->attn_q->abs_offset,
            2u * DS4_EAGLE3_N_EMBD, q_dim, g->eagle3_input, 1) != 0;
    if (ok) ok = ds4_gpu_matmul_f16_pair_tensor(
            g->eagle3_k, g->eagle3_v,
            model->map, model->size,
            w->attn_k->abs_offset, w->attn_v->abs_offset,
            2u * DS4_EAGLE3_N_EMBD, kv_dim, g->eagle3_input, 1) != 0;
    if (ok) ok = ds4_gpu_qwen_rope_tensor(
            g->eagle3_q, DS4_N_HEAD, DS4_N_HEAD_DIM, pos, 1000000.0f) != 0;
    if (ok) ok = ds4_gpu_qwen_rope_tensor(
            g->eagle3_k, DS4_N_HEAD_KV, DS4_N_HEAD_DIM, pos, 1000000.0f) != 0;
    if (ok) ok = ds4_gpu_qwen_store_kv_tensor(
            g->eagle3_k_cache, g->eagle3_v_cache,
            g->eagle3_k, g->eagle3_v,
            pos, g->raw_cap, (uint32_t)kv_dim, DS4_N_HEAD_DIM) != 0;
    if (ok) ok = ds4_gpu_qwen_attention_tensor(
            g->eagle3_heads, g->eagle3_q,
            g->eagle3_k_cache, g->eagle3_v_cache,
            pos + 1u, g->raw_cap,
            DS4_N_HEAD, DS4_N_HEAD_KV, DS4_N_HEAD_DIM) != 0;
    if (ok) ok = ds4_gpu_matmul_f16_tensor(
            g->eagle3_attn_out, model->map, model->size, w->attn_output->abs_offset,
            q_dim, DS4_EAGLE3_N_EMBD, g->eagle3_heads, 1) != 0;
    if (ok) ok = ds4_gpu_add_tensor(
            g->eagle3_after_attn, g_input, g->eagle3_attn_out,
            DS4_EAGLE3_N_EMBD) != 0;
    if (ok) ok = ds4_gpu_rms_norm_weight_tensor(
            g->eagle3_ffn_norm, g->eagle3_after_attn,
            model->map, model->size, w->ffn_norm->abs_offset,
            DS4_EAGLE3_N_EMBD, DS4_RMS_EPS) != 0;
    if (ok) ok = ds4_gpu_matmul_f16_pair_tensor(
            g->eagle3_gate, g->eagle3_up,
            model->map, model->size,
            w->ffn_gate->abs_offset, w->ffn_up->abs_offset,
            DS4_EAGLE3_N_EMBD, DS4_EAGLE3_N_FF,
            g->eagle3_ffn_norm, 1) != 0;
    if (ok) ok = ds4_gpu_swiglu_tensor(
            g->eagle3_mid, g->eagle3_gate, g->eagle3_up,
            DS4_EAGLE3_N_FF, 0.0f, 1.0f) != 0;
    if (ok) ok = ds4_gpu_matmul_f16_tensor(
            g->eagle3_down, model->map, model->size, w->ffn_down->abs_offset,
            DS4_EAGLE3_N_FF, DS4_EAGLE3_N_EMBD, g->eagle3_mid, 1) != 0;
    if (ok) ok = ds4_gpu_add_tensor(
            out_state, g->eagle3_after_attn, g->eagle3_down,
            DS4_EAGLE3_N_EMBD) != 0;
    if (ok && need_top) ok = ds4_gpu_rms_norm_weight_tensor(
            g->eagle3_output_norm, out_state,
            model->map, model->size, w->output_norm->abs_offset,
            DS4_EAGLE3_N_EMBD, DS4_RMS_EPS) != 0;
    if (ok && need_top) ok = ds4_gpu_matmul_f16_tensor(
            g->eagle3_logits, model->map, model->size, w->output->abs_offset,
            DS4_EAGLE3_N_EMBD, DS4_EAGLE3_N_VOCAB,
            g->eagle3_output_norm, 1) != 0;
    if (ok && need_top) ok = ds4_gpu_indexer_topk_tensor(
            g->comp_selected, g->eagle3_logits,
            DS4_EAGLE3_N_VOCAB, 1, 1) != 0;
    return ok;
}

static bool metal_graph_eval_eagle3_decode(
        ds4_gpu_graph *g,
        const ds4_model *base_model,
        const ds4_weights *base_weights,
        const ds4_gpu_tensor *g_input,
    ...
    if (ok) ok = metal_graph_encode_eagle3_decode(
            g, base_model, base_weights, g_input, out_state, token, pos, need_top);
    if (ok) ok = ds4_gpu_end_commands() != 0;
    else (void)ds4_gpu_synchronize();
    if (ok && need_top) {
        int draft_top = -1;
        ok = ds4_gpu_tensor_read(g->comp_selected, 0, &draft_top, sizeof(draft_top)) != 0;
        if (ok && (draft_top < 0 || draft_top >= DS4_EAGLE3_N_VOCAB)) ok = false;
        if (ok && target_top) {
            const int64_t *d2t = tensor_data(g->eagle3_model, g->eagle3_weights->d2t);
            const int64_t mapped = (int64_t)draft_top + d2t[draft_top];
            ok = mapped >= 0 && mapped < DS4_N_VOCAB;
            if (ok) *target_top = (int)mapped;
        }
    }
    if (ok && g->mtp_n_raw < pos + 1u) g->mtp_n_raw = pos + 1u;
    return ok;
}

static bool metal_graph_eagle3_set_pending_batch_row(
        ds4_gpu_graph *g,
        uint32_t row) {
    if (!g || !g->eagle3_weights || row >= g->prefill_cap) return false;
    const uint64_t row_bytes = (uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float);
    bool ok = ds4_gpu_begin_commands() != 0;
    if (ok) {
        ok = ds4_gpu_tensor_copy(
                g->eagle3_pending_g, 0,
                g->eagle3_batch_g, (uint64_t)row * row_bytes,
                row_bytes) != 0;
    }
    if (ok) ok = ds4_gpu_end_commands() != 0;
    else (void)ds4_gpu_synchronize();
...
static bool metal_graph_eagle3_ingest_target_batch(
        ds4_gpu_graph *g,
        const ds4_model *base_model,
        const ds4_weights *base_weights,
        ...
        uint32_t n_tokens) {
    if (!g || !g->eagle3_weights) return true;
    if (!tokens || n_tokens == 0 || n_tokens > g->prefill_cap ||
        pos0 > (uint32_t)tokens->len ||
        n_tokens > (uint32_t)tokens->len - pos0) {
        return false;
    }
    const uint64_t row_bytes = (uint64_t)DS4_EAGLE3_N_EMBD * sizeof(float);
    bool ok = true;
    if (g->eagle3_pending_valid && pos0 > 0) {
        ok = metal_graph_eval_eagle3_decode(
                g, base_model, base_weights,
                g->eagle3_pending_g, g->mtp_state_hc,
                tokens->v[pos0], pos0 - 1u, false, NULL);
    }
    for (uint32_t row = 0; ok && row + 1u < n_tokens; row++) {
        ds4_gpu_tensor *g_row = ds4_gpu_tensor_view(
                g->eagle3_batch_g, (uint64_t)row * row_bytes, row_bytes);
        ok = g_row != NULL && metal_graph_eval_eagle3_decode(
                g, base_model, base_weights,
                g_row, g->mtp_state_hc,
                tokens->v[pos0 + row + 1u], pos0 + row, false, NULL);
        ds4_gpu_tensor_free(g_row);
    }
    if (ok) ok = metal_graph_eagle3_set_pending_batch_row(g, n_tokens - 1u);
    return ok;
}

static bool qwen_graph_embed_token(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_weights *weights,
        int token) {
    const ds4_tensor *te = weights->token_embd;
    if (!te || token < 0 || (uint64_t)token >= te->dim[1]) return false;
    if (te->type != DS4_TENSOR_Q4_K && te->type != DS4_TENSOR_Q6_K) {
        fprintf(stderr, "ds4: Qwen Metal expects K-quant token embeddings\n");
        return false;
    }
    const uint64_t row_bytes = te->bytes / te->dim[1];
    return ds4_gpu_dequant_k_row_tensor(g->cur_hc,
                                        model->map,
                                        model->size,
                                        te->abs_offset + (uint64_t)token * row_bytes,
                                        te->type,
...
static bool qwen_graph_encode_output_logits(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_weights *weights) {
    const uint64_t vocab_dim = weights->output->dim[1];
...
static bool qwen_graph_encode_output_top1(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_weights *weights) {
    bool ok = ds4_gpu_rms_norm_weight_tensor(g->output_norm,
                                             ...
                                             DS4_N_EMBD,
                                             DS4_RMS_EPS) != 0;
    if (ok) ok = ds4_gpu_matmul_k_argmax_tensor(g->output_pre,
                                                g->top_id,
                                                model->map,
                                                model->size,
                                                weights->output->abs_offset,
...
static bool qwen_graph_update_rope_tables(ds4_gpu_graph *g, uint32_t pos) {
    if (!g || !g->rope_cos || !g->rope_sin) return false;
    enum { HALF_DIM = DS4_N_HEAD_DIM / 2 };
    const uint64_t need_bytes = ((uint64_t)pos + 1u) * HALF_DIM * sizeof(float);
    if (ds4_gpu_tensor_bytes(g->rope_cos) >= need_bytes &&
        ds4_gpu_tensor_bytes(g->rope_sin) >= need_bytes) {
        return true;
    }
    float c[HALF_DIM];
    float s[HALF_DIM];
    ...
    return ds4_gpu_tensor_write(g->rope_cos, 0, c, sizeof(c)) != 0 &&
           ds4_gpu_tensor_write(g->rope_sin, 0, s, sizeof(s)) != 0;
}

static bool qwen_token_stage_profile_enabled(void) {
    static int enabled = -1;
    if (enabled < 0) enabled = getenv("DS4_QWEN_TOKEN_STAGE_PROFILE") != NULL;
    return enabled != 0;
}

static bool qwen_token_async_stage_profile_enabled(void) {
    static int enabled = -1;
    if (enabled < 0) enabled = getenv("DS4_QWEN_TOKEN_ASYNC_STAGE_PROFILE") != NULL;
    return enabled != 0;
}

static uint32_t qwen_token_split_after_layers(void) {
...
static bool qwen_graph_encode_token(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_weights *weights,
        int token,
    ...
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    const bool qwen_async_stage_profile = qwen_token_async_stage_profile_enabled();
    const bool qwen_stage_profile =
        qwen_token_stage_profile_enabled() || qwen_async_stage_profile;
...
#define DS4_PROFILE_QWEN_TOKEN_STAGE(layer_id, name) do { \
        if (ok && qwen_stage_profile) { \
            if (qwen_async_stage_profile) { \
                char profile_label[128]; \
                snprintf(profile_label, sizeof(profile_label), \
                         "pos=%u layer=%d %s", pos, (int)(layer_id), (name)); \
                if (ds4_gpu_flush_commands_profile(profile_label) == 0) ok = false; \
                break; \
            } \
    ...
    bool ok = qwen_graph_update_rope_tables(g, pos);
    if (ok) ok = qwen_graph_embed_token(g, model, weights, token);
    DS4_PROFILE_QWEN_TOKEN_STAGE(-1, "embed");

    const uint32_t split_after_layers = qwen_token_split_after_layers();

    bool attn_norm_ready = false;
    for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
        const ds4_layer_weights *layer = &weights->layer[il];
        const uint32_t n_active_experts =
            g->quality ? DS4_N_EXPERT_USED : DS4_N_EXPERT_ACTIVE;
        const uint64_t gate_row_bytes = routed_expert_row_bytes(layer->ffn_gate_exps);
        ...
        if (ok) ok = metal_graph_eagle3_capture_token_input(g, il);
        if (!attn_norm_ready) {
        ...
        if (ok && layer->attn_q_a->type == layer->attn_q_b->type) {
            ok = ds4_gpu_qwen_qkv_k_tensor(
                    g->q,
                    g->kv_raw,
                    g->kv,
                    model->map,
        ...
        if (ok) ok = ds4_gpu_qwen_norm_rope_weight_store_kv_tensor(g->q,
        ...
        if (ok) ok = ds4_gpu_qwen_attention_tensor(g->heads,
                                                   ...
                                                   DS4_N_HEAD_KV,
                                                   DS4_N_HEAD_DIM) != 0;
        DS4_PROFILE_QWEN_TOKEN_STAGE((int)il, "attention");
        ...
        DS4_PROFILE_QWEN_TOKEN_STAGE((int)il, "attention_projection");
        ...
        DS4_PROFILE_QWEN_TOKEN_STAGE((int)il, "attention_resid_norm");

        const bool router_f16 = !g->quality && g->router_cache_f16 != NULL;
        const uint64_t router_elem_bytes = router_f16 ? sizeof(uint16_t) : sizeof(float);
        if (ok) ok = ds4_gpu_qwen_router_cached_tensor(
                g->router_logits,
                router_f16 ? g->router_cache_f16 : g->router_cache,
                (uint64_t)il * DS4_N_EMBD * DS4_N_EXPERT * router_elem_bytes,
                g->ffn_norm,
                router_f16) != 0;
        DS4_PROFILE_QWEN_TOKEN_STAGE((int)il, "router_matvec");
        if (ok) ok = ds4_gpu_qwen_router_select_tensor(g->router_selected,
                                                       ...
                                                       n_active_experts) != 0;
        if (ok) {
            metal_graph_debug_dump_tensor("router_logits", g->router_logits, DS4_N_EXPERT, il, pos);
            metal_graph_debug_dump_tensor("router_weights", g->router_weights, n_active_experts, il, pos);
            metal_graph_debug_dump_i32_tensor("router_selected", g->router_selected, n_active_experts, il, pos);
        }
        DS4_PROFILE_QWEN_TOKEN_STAGE((int)il, "router_select");
        if (ok) ok = ds4_gpu_routed_moe_batch_tensor(g->routed_out,
                                                     ...
                                                     NULL) != 0;
        if (ok) {
            const uint64_t routed_mid_elems = (uint64_t)n_active_experts * DS4_N_FF_EXP;
            metal_graph_debug_dump_tensor("moe_gate", g->routed_gate, routed_mid_elems, il, pos);
            metal_graph_debug_dump_tensor("moe_up", g->routed_up, routed_mid_elems, il, pos);
            metal_graph_debug_dump_tensor("moe_mid", g->routed_mid, routed_mid_elems, il, pos);
            metal_graph_debug_dump_tensor("moe_down", g->routed_down,
                                          (uint64_t)n_active_experts * DS4_N_EMBD, il, pos);
        }
        ...
        if (ok) metal_graph_debug_dump_tensor("resid", g->cur_hc, DS4_N_EMBD, il, pos);
    ...
    if (need_logits) {
        if (ok) ok = qwen_graph_encode_output_logits(g, model, weights);
        DS4_PROFILE_QWEN_TOKEN_STAGE(-1, "output_head");
    }
    if (ok) ok = metal_graph_eagle3_finish_token_features(g);
#undef DS4_PROFILE_QWEN_TOKEN_STAGE
    return ok;
}

static bool qwen_graph_eval_token(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_weights *weights,
        int token,
...
static bool qwen_graph_eval_token_top1(
        ...
        uint32_t pos,
        int *top_id) {
    if (!top_id || !g || !g->top_id) return false;
    const bool profile_top1 = getenv("DS4_QWEN_TOP1_PROFILE") != NULL;
    const double t0 = profile_top1 ? now_sec() : 0.0;
    bool ok = ds4_gpu_begin_commands() != 0;
    if (ok) ok = qwen_graph_encode_token(g, model, weights, token, pos, false);
    if (ok && profile_top1) {
        const double encoded = now_sec();
        ok = ds4_gpu_flush_commands() != 0;
        if (ok) ok = ds4_gpu_synchronize() != 0;
        const double layers_done = now_sec();
        fprintf(stderr,
                "ds4: qwen top1 profile pos=%u layers encode=%.3f ms execute=%.3f ms total=%.3f ms\n",
                pos,
                (encoded - t0) * 1000.0,
                (layers_done - encoded) * 1000.0,
                (layers_done - t0) * 1000.0);
        if (ok) ok = ds4_gpu_begin_commands() != 0;
    }
    const double out_t0 = profile_top1 ? now_sec() : 0.0;
    if (ok) ok = qwen_graph_encode_output_top1(g, model, weights);
    if (ok && profile_top1) {
        const double encoded = now_sec();
        ok = ds4_gpu_end_commands() != 0;
        const double out_done = now_sec();
        fprintf(stderr,
                "ds4: qwen top1 profile pos=%u output encode=%.3f ms execute=%.3f ms total=%.3f ms\n",
                pos,
                (encoded - out_t0) * 1000.0,
                (out_done - encoded) * 1000.0,
                (out_done - out_t0) * 1000.0);
    }
    if (ok && !profile_top1) ok = ds4_gpu_end_commands() != 0;
    if (!ok) {
        (void)ds4_gpu_end_commands();
        return false;
    }
    return ds4_gpu_tensor_read(g->top_id, 0, top_id, sizeof(*top_id)) != 0;
}
...
static bool qwen_graph_upload_prompt_embeddings(
        ds4_gpu_tensor   *out,
        const ds4_model    *model,
        const ds4_weights  *weights,
        const token_vec    *prompt,
        uint32_t            start,
        uint32_t            n_tokens) {
    if (!out || !model || !weights || !prompt) return false;
    if (start > (uint32_t)prompt->len || n_tokens > (uint32_t)prompt->len - start) return false;
    const uint64_t total = (uint64_t)n_tokens * DS4_N_EMBD;
    float *rows = xmalloc((size_t)total * sizeof(rows[0]));
    for (uint32_t t = 0; t < n_tokens; t++) {
        embed_token_any(model, weights, prompt->v[start + t],
                        rows + (uint64_t)t * DS4_N_EMBD);
    }
    const bool ok = ds4_gpu_tensor_write(out, 0, rows, total * sizeof(rows[0])) != 0;
    free(rows);
    return ok;
}

static bool qwen_graph_encode_layer_batch(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_layer_weights *layer,
        uint32_t il,
        uint32_t pos0,
        uint32_t n_tokens) {
    if (!g || !model || !layer || n_tokens == 0 || n_tokens > g->prefill_cap) return false;
    if (pos0 > g->raw_cap || n_tokens > g->raw_cap - pos0) return false;
    ...
    const bool batch_async_profile =
        n_tokens <= DS4_MAX_DRAFT_TOKENS &&
        getenv("DS4_QWEN_BATCH_ASYNC_STAGE_PROFILE") != NULL;
#define DS4_PROFILE_QWEN_BATCH_STAGE(name) do { \
        if (ok && batch_async_profile) { \
            char profile_label[128]; \
            snprintf(profile_label, sizeof(profile_label), \
                     "qwen_batch layer=%u tokens=%u %s", il, n_tokens, (name)); \
            if (ds4_gpu_flush_commands_profile(profile_label) == 0) ok = false; \
        } \
    } while (0)

    const char *stage = "attn_norm";
    bool ok = metal_graph_eagle3_capture_batch_input(g, il, n_tokens);
    if (ok) ok = ds4_gpu_rms_norm_weight_rows_tensor(g->batch_attn_norm,
                                                  ...
                                                  n_tokens,
                                                  DS4_RMS_EPS) != 0;
    DS4_PROFILE_QWEN_BATCH_STAGE("attn_norm");
    if (ok) { stage = "q_proj"; ok = metal_graph_matmul_plain_tensor(g->batch_q, model, layer->attn_q_a,
                                                 DS4_N_EMBD, q_dim, g->batch_attn_norm, n_tokens);
    }
    if (ok) { stage = "k_proj"; ok = metal_graph_matmul_plain_tensor(g->batch_kv_raw, model, layer->attn_q_b,
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("qkv");
    if (ok && g_lora_scale != 0.0f) {
        ds4_gpu_tensor *lora_A = NULL, *lora_B = NULL;
        uint32_t lora_rank = 0, lora_in = 0, lora_out = 0;
        const struct {
    ...
    if (ok) { stage = "k_norm"; ok = ds4_gpu_qwen_head_rms_norm_weight_tensor(g->batch_kv_raw,
                                                           model->map,
                                                           model->size,
                                                           layer->attn_kv_a_norm->abs_offset,
                                                           DS4_N_HEAD_KV * n_tokens,
                                                           DS4_N_HEAD_DIM,
                                                           DS4_RMS_EPS) != 0;
    }
    if (ok) { stage = "q_rope"; ok = ds4_gpu_qwen_rope_batch_tensor(g->batch_q, n_tokens, DS4_N_HEAD,
                                                DS4_N_HEAD_DIM, pos0, DS4_ROPE_FREQ_BASE) != 0;
    }
    if (ok) { stage = "k_rope"; ok = ds4_gpu_qwen_rope_batch_tensor(g->batch_kv_raw, n_tokens, DS4_N_HEAD_KV,
                                                DS4_N_HEAD_DIM, pos0, DS4_ROPE_FREQ_BASE) != 0;
    }
    DS4_PROFILE_QWEN_BATCH_STAGE("qk_norm_rope");
    if (ok) { stage = "kv_store"; ok = ds4_gpu_qwen_store_kv_batch_tensor(g->layer_raw_cache[il],
                                                    g->layer_v_cache[il],
                                                    g->batch_kv_raw,
                                                    g->batch_kv,
                                                    pos0,
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("kv_store");
    if (ok) {
        stage = "attention";
        if (n_tokens <= 4) {
            ...
            ok = ds4_gpu_qwen_attention_batch_tensor(g->batch_heads,
                                                     g->batch_q,
                                                     g->layer_raw_cache[il],
                                                     g->layer_v_cache[il],
                                                     pos0,
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("attention");
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("attn_out");
    if (ok) {
    ...
    if (ok) { stage = "ffn_norm"; ok = ds4_gpu_rms_norm_weight_rows_tensor(g->batch_ffn_norm,
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("attn_resid_ffn_norm");
    if (ok) {
        stage = "router_logits";
        if (n_tokens <= 4) {
            const bool router_f16 = !g->quality && g->router_cache_f16 != NULL;
            const uint64_t router_elem_bytes = router_f16 ? sizeof(uint16_t) : sizeof(float);
            for (uint32_t row = 0; ok && row < n_tokens; row++) {
                ds4_gpu_tensor *x_row = metal_graph_tensor_row_view(
                        g->batch_ffn_norm, row, DS4_N_EMBD);
                ds4_gpu_tensor *out_row = metal_graph_tensor_row_view(
                        g->batch_router_logits, row, DS4_N_EXPERT);
                ok = x_row && out_row &&
                     ds4_gpu_qwen_router_cached_tensor(
                             out_row,
                             router_f16 ? g->router_cache_f16 : g->router_cache,
                             (uint64_t)il * DS4_N_EMBD * DS4_N_EXPERT * router_elem_bytes,
                             x_row,
            ...
            ok = metal_graph_matmul_plain_tensor(g->batch_router_logits, model, layer->ffn_gate_inp,
        ...
        if (n_tokens <= 4) {
            for (uint32_t row = 0; ok && row < n_tokens; row++) {
                ds4_gpu_tensor *selected_row = metal_graph_tensor_row_view(
                        g->batch_router_selected, row, n_active_experts);
                ds4_gpu_tensor *weights_row = metal_graph_tensor_row_view(
                        g->batch_router_weights, row, n_active_experts);
                ds4_gpu_tensor *probs_row = metal_graph_tensor_row_view(
                        g->batch_router_probs, row, DS4_N_EXPERT);
                ds4_gpu_tensor *logits_row = metal_graph_tensor_row_view(
                        g->batch_router_logits, row, DS4_N_EXPERT);
                ok = selected_row && weights_row && probs_row && logits_row &&
                     ds4_gpu_qwen_router_select_tensor(
                             selected_row, weights_row, probs_row, logits_row,
                             DS4_N_EXPERT, n_active_experts) != 0;
            ...
            ok = ds4_gpu_qwen_router_select_batch_tensor(g->batch_router_selected,
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("router");
    if (ok) {
        stage = "routed_moe";
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("routed_moe");
    if (ok && metal_graph_directional_steering_ffn_enabled(g)) {
    ...
    DS4_PROFILE_QWEN_BATCH_STAGE("ffn_resid");
    if (ok) {
    ...
    const uint32_t split_after_layers = qwen_token_split_after_layers();
    if (ok && n_tokens <= 4u && split_after_layers != 0u &&
        il + 1u < DS4_N_LAYER &&
        ((il + 1u) % split_after_layers) == 0u) {
        ok = ds4_gpu_flush_commands() != 0;
    }
    if (!ok) {
        fprintf(stderr, "ds4: Qwen batch prefill failed at layer %u stage %s\n", il, stage);
    }
#undef DS4_PROFILE_QWEN_BATCH_STAGE
    return ok;
}

static bool qwen_graph_prefill_batch_tokens(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_weights *weights,
        const token_vec *prompt,
        ...
        void *progress_ud) {
    if (!g || !model || !weights || !prompt || n_tokens == 0 || g->prefill_cap == 0) return false;
    if (start > (uint32_t)prompt->len || n_tokens > (uint32_t)prompt->len - start) return false;
    if (start + n_tokens > g->raw_cap) return false;

    uint32_t done = 0;
    while (done < n_tokens) {
        const uint32_t chunk = (n_tokens - done) < g->prefill_cap ?
                               (n_tokens - done) : g->prefill_cap;
        const uint32_t pos0 = start + done;
        bool ok = qwen_graph_upload_prompt_embeddings(g->batch_cur_hc,
                                                      model,
                                                      weights,
                                                      prompt,
                                                      pos0,
        ...
        if (ok) ok = ds4_gpu_begin_commands() != 0;
        for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
            ok = qwen_graph_encode_layer_batch(g, model, &weights->layer[il], il, pos0, chunk);
        }
        if (ok) ok = metal_graph_eagle3_finish_batch_features(g, chunk);

        ds4_gpu_tensor *last_hc = NULL;
        ds4_gpu_tensor *saved_cur = g->cur_hc;
        const bool need_logits = logits_out && done + chunk == n_tokens;
        if (ok && need_logits) {
        ...
        if (ok && need_logits) {
            ...
            ok = ds4_gpu_rms_norm_weight_tensor(g->output_norm,
                                                ...
                                                DS4_RMS_EPS) != 0;
            if (ok) ok = metal_graph_matmul_plain_tensor(g->logits, model, weights->output,
                                                         DS4_N_EMBD, weights->output->dim[1],
                                                         g->output_norm, 1);
            g->cur_hc = saved_cur;
        }
        if (ok) ok = ds4_gpu_end_commands() != 0;
        ...
        if (last_hc) ds4_gpu_tensor_free(last_hc);
        if (ok) {
            ok = metal_graph_eagle3_ingest_target_batch(
                    g, model, weights, prompt, pos0, chunk);
        }
        if (!ok) return false;
        if (progress) progress(progress_ud, "prefill_chunk", (int)(pos0 + chunk), prompt->len);
        done += chunk;
    }

    if (logits_out) {
        return ds4_gpu_tensor_read(g->logits, 0, logits_out,
                                   (uint64_t)DS4_N_VOCAB * sizeof(float)) != 0;
    }
    return true;
}

static bool qwen_graph_prefill_tokens(
        ds4_gpu_graph *g,
        const ds4_model *model,
        const ds4_weights *weights,
        const token_vec *prompt,
        ...
        ds4_session_progress_fn progress,
        void *progress_ud) {
    if (!g || !model || !weights || !prompt || n_tokens == 0) return false;
    if (start > (uint32_t)prompt->len || n_tokens > (uint32_t)prompt->len - start) return false;
    if (start + n_tokens > g->raw_cap) return false;
    if (n_tokens > 1 && g->prefill_cap > 1 && !g->quality) {
        return qwen_graph_prefill_batch_tokens(g, model, weights, prompt, start,
                                               n_tokens, logits_out, progress, progress_ud);
    }

    const uint32_t end = start + n_tokens;
    const uint32_t flush_every = 8;
    bool ok = ds4_gpu_begin_commands() != 0;
    for (uint32_t pos = start; ok && pos < end; pos++) {
        if (g->eagle3_weights && g->eagle3_pending_valid && pos > 0) {
            ok = metal_graph_encode_eagle3_decode(
                    g, model, weights,
                    g->eagle3_pending_g, g->mtp_state_hc,
                    prompt->v[pos], pos - 1u, false);
        }
        if (ok) ok = qwen_graph_encode_token(g, model, weights, prompt->v[pos], pos, pos + 1 == end);
        if (progress) progress(progress_ud, "prefill_chunk", (int)(pos + 1), prompt->len);
        if (ok && pos + 1 < end && ((pos - start + 1) % flush_every) == 0)
            ok = ds4_gpu_flush_commands() != 0;
    }
    if (ok) ok = ds4_gpu_end_commands() != 0;
    if (!ok) {
...
static bool metal_graph_eval_mtp_draft_from_hc(
        ...
        int                   *top_id) {
    (void)logits;
    return metal_graph_eval_eagle3_decode(
            g, base_model, base_weights,
            prev_hc, out_hc, token, pos, true, top_id);
}
...
static bool qwen_graph_prefill_imatrix(
        ds4_gpu_graph         *g,
        const ds4_model       *model,
        const ds4_weights     *weights,
        const token_vec       *prompt,
    ...
    if (!g || !model || !weights || !prompt || !imatrix ||
        n_tokens == 0 || n_tokens > (uint32_t)prompt->len || g->prefill_cap == 0) {
        return false;
    }

    for (uint32_t pos0 = 0; pos0 < n_tokens; ) {
        const uint32_t remaining = n_tokens - pos0;
        const uint32_t chunk = remaining < g->prefill_cap ? remaining : g->prefill_cap;
        bool ok = qwen_graph_upload_prompt_embeddings(g->batch_cur_hc,
                                                      model,
                                                      weights,
                                                      prompt,
        ...
        for (uint32_t il = 0; ok && il < DS4_N_LAYER; ++il) {
            ok = ds4_gpu_begin_commands() != 0;
            if (ok) {
                ok = qwen_graph_encode_layer_batch(g,
                                                   model,
                                                   &weights->layer[il],
                                                   il,
                                                   pos0,
            ...
            ok = imatrix_collect_layer_batch(imatrix, g, il, chunk);
        }
        if (!ok) return false;
        pos0 += chunk;
    }
...
static bool metal_graph_prefill_layer_major(
    ...
    if (!split_commands) {
        ...
        if (ok) ok = metal_graph_eagle3_finish_batch_features(g, n_tokens);
...
static bool metal_graph_verify_suffix_tops(
        ...
        int                   *row_tops,
        float                 *row_logits) {
    const bool trace = getenv("DS4_MTP_SPEC_LOG") != NULL;
    ...
    if (ok) {
        stage = "eagle_features";
        ok = metal_graph_eagle3_finish_batch_features(g, n_tokens);
    }
    if (ok) {
        stage = "end_layers";
        ok = ds4_gpu_end_commands() != 0;
...
fail:
    if (trace) {
        if (failed_layer == UINT32_MAX) {
            fprintf(stderr,
                    "ds4: mtp verifier failed stage=%s start=%u tokens=%u\n",
                    stage, start, n_tokens);
        } else {
            fprintf(stderr,
                    "ds4: mtp verifier failed stage=%s layer=%u start=%u tokens=%u\n",
                    stage, failed_layer, start, n_tokens);
        }
    }
    return false;
}
...
/* Qwen3-Coder has full attention, so the Metal raw K/V cache is the context. */
static uint32_t metal_graph_raw_cap_for_context(int ctx_size, uint32_t prefill_cap) {
    (void)prefill_cap;
    return ctx_size > 0 ? (uint32_t)ctx_size : 1u;
}
...
ds4_context_memory ds4_context_memory_estimate(ds4_backend backend, int ctx_size) {
    ...
    if (ds4_backend_uses_graph(backend)) {
        ...
        m.raw_cap = ctx;
        m.comp_cap = 0;
        const uint64_t qwen_kv_dim = (uint64_t)DS4_N_HEAD_KV * DS4_N_HEAD_DIM;
        ...
        const uint64_t single_f32 =
            ...
            qwen_q_dim * 2u +
            qwen_kv_dim * 2u +
            (uint64_t)DS4_N_EXPERT * 3u +
            (uint64_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP * 3u +
            (uint64_t)DS4_N_VOCAB;
        const uint64_t batch_f32_per_token =
            (uint64_t)DS4_N_EMBD * 7u +
            qwen_q_dim * 2u +
            qwen_kv_dim * 2u +
            (uint64_t)DS4_N_EXPERT * 2u +
            (uint64_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP * 3u;
        const uint64_t batch_i32 =
            (uint64_t)m.prefill_cap *
            ((uint64_t)DS4_N_EXPERT_USED + 1u) *
            sizeof(int32_t);
        m.scratch_bytes = single_f32 * sizeof(float) +
                          batch_f32_per_token * (uint64_t)m.prefill_cap * sizeof(float) +
                          batch_i32;
    } else {
        ...
        m.raw_bytes = (uint64_t)DS4_N_LAYER *
                      m.raw_cap *
                      ((uint64_t)DS4_N_HEAD_DIM + 2u * qwen_kv_dim) *
                      sizeof(float);
        for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
            const uint32_t ratio = ds4_layer_compress_ratio(il);
            if (ratio == 0) continue;
...
struct ds4_engine {
    ...
    ds4_vocab vocab;
    ds4_weights weights;
    ds4_eagle3_weights eagle3_weights;
    ds4_backend backend;
    ...
    float directional_steering_ffn_scale;
    /* Multiple vector support */
    char *steering_files[DS4_MAX_STEERING_VECTORS];
...
static void cpu_directional_steering_project_rows_multi(
        ...
        const float  *scales) {
    for (int i = 0; i < n; i++) {
        cpu_directional_steering_project_rows(x, dirs[i], il, rows, scales[i]);
    }
}

static bool cpu_load_directional_steering_single(
        ds4_engine *e,
        int         idx) {
    if (e->steering_attn_scales[idx] == 0.0f &&
        e->steering_ffn_scales[idx] == 0.0f) {
        return true;
    }

    const char *path = e->steering_files[idx];
    if (!path || !path[0]) {
        fprintf(stderr, "ds4: directional steering vector %d needs a file\n", idx);
        return false;
    }

    const uint64_t n = (uint64_t)DS4_N_LAYER * DS4_N_EMBD;
    e->steering_dirs[idx] = xmalloc((size_t)n * sizeof(e->steering_dirs[idx][0]));
    if (!read_f32_binary_file(path, e->steering_dirs[idx], n)) {
        free(e->steering_dirs[idx]);
    ...
    fprintf(stderr, "ds4: CPU directional steering enabled: %s attn=%g ffn=%g\n",
            path,
            (double)e->steering_attn_scales[idx],
            (double)e->steering_ffn_scales[idx]);
    return true;
}

static bool cpu_load_directional_steering(ds4_engine *e) {
    if (!e || e->n_steering_vectors <= 0) return true;

    for (int i = 0; i < e->n_steering_vectors; i++) {
        if (!cpu_load_directional_steering_single(e, i)) return false;
    }
    /* Legacy pointer for backward compat */
    if (e->n_steering_vectors > 0) {
        e->directional_steering_dirs = e->steering_dirs[0];
    }
    return true;
}
...
static void vocab_load(ds4_vocab *vocab, const ds4_model *model) {
    ...
    vocab->assistant_id = vocab->user_id;
...
/* Build the Qwen3-Coder chat prompt using the ChatML-style template:
 * <|im_start|>role\ncontent<|im_end|>\n ... <|im_start|>assistant\n */
...
/* CPU generation entry point.  It prefills one token at a time into the Qwen
 * K/V cache, then decodes from that same cache. */
static int generate_raw_swa_cpu(
    ...
    (void)progress;
    (void)progress_ud;
    fprintf(stderr, "ds4: using Qwen CPU generation with native K/V cache\n");
    ...
    for (uint64_t t = 0; t < (uint64_t)prompt->len; t++) {
...
static int generate_metal_graph_raw_swa(
    ...
    fprintf(stderr, "ds4: using Qwen3-Coder Metal generation\n");
    const double t_prefill0 = now_sec();
    ok = qwen_graph_prefill_tokens(&g, model, weights, prompt, 0,
    ...
    const bool greedy_top1 = !trace_top;
    int cached_top1 = sample_argmax(logits, DS4_N_VOCAB);
    int pos = prompt->len;
    int n_generated = 0;
    int n_decode_eval = 0;
    const double t_decode0 = now_sec();
    for (int i = 0; i < n_predict && pos < ctx_size; i++) {
        ...
        int token = greedy_top1 ? cached_top1 : sample_argmax(logits, DS4_N_VOCAB);
        if (token == vocab->eos_id) break;
        if (emit) emit(emit_ud, token);
        n_generated++;
        if (i == n_predict - 1 || pos + 1 >= ctx_size) {
        ...
        const double t_eval0 = token_timing ? now_sec() : 0.0;
        if (greedy_top1) {
            ok = qwen_graph_eval_token_top1(&g, model, weights, token,
                                            (uint32_t)pos, &cached_top1);
        } else {
            ok = qwen_graph_eval_token(&g, model, weights, token,
                                       (uint32_t)pos, logits);
        }
        if (!ok) break;
        if (token_timing) {
            const double t_eval1 = now_sec();
            fprintf(stderr, "ds4: qwen metal decode eval %d took %.3f ms\n",
                    n_decode_eval + 1, (t_eval1 - t_eval0) * 1000.0);
        }
        n_decode_eval++;
        pos++;
...
struct ds4_session {
    ...
    uint32_t prefill_cap;
    int ctx_size;
    bool quality_override_valid;
    bool quality_override;
...
static void session_note_logits_valid(ds4_session *s) {
    if (!s) return;
    s->logits_valid = true;
    s->cached_argmax_valid = false;
}

static void session_note_logits_stale_top1(ds4_session *s, int top_id) {
    if (!s) return;
    s->logits_valid = false;
    s->cached_argmax = top_id;
    s->cached_argmax_valid = top_id >= 0;
}

static void session_note_logits_invalid(ds4_session *s) {
    if (!s) return;
    s->logits_valid = false;
    s->cached_argmax_valid = false;
    s->cached_argmax = -1;
...
static bool session_effective_quality(const ds4_session *s) {
    if (!s || !s->engine) return false;
    return s->quality_override_valid ? s->quality_override : s->engine->quality;
}

int ds4_session_set_quality(ds4_session *s, bool quality) {
    if (!s) return 1;
    s->quality_override_valid = true;
    s->quality_override = quality;
#ifndef DS4_NO_GPU
    if (!ds4_session_is_cpu(s) && s->graph.quality != quality) {
        s->graph.quality = quality;
...
#endif
    if (s->cfg_session) {
        (void)ds4_session_set_quality(s->cfg_session, quality);
    }
    return 0;
}

int ds4_session_clear_quality_override(ds4_session *s) {
    if (!s) return 1;
    const bool quality = s->engine ? s->engine->quality : false;
    s->quality_override_valid = false;
    s->quality_override = false;
#ifndef DS4_NO_GPU
    if (!ds4_session_is_cpu(s) && s->graph.quality != quality) {
        s->graph.quality = quality;
        s->checkpoint_valid = false;
...
#endif
    if (s->cfg_session) {
        (void)ds4_session_clear_quality_override(s->cfg_session);
    }
    return 0;
}

static int session_materialize_logits(ds4_session *s, char *err, size_t errlen) {
    if (!s) return 1;
...
#ifdef DS4_NO_GPU
    payload_set_err(err, errlen, "GPU support is not compiled in");
    return 1;
#else
    ds4_engine *e = s->engine;
    if (!e || !s->checkpoint_valid || !s->graph.logits) {
        payload_set_err(err, errlen, "session has no materialized Metal logits");
        return 1;
    }
    if (ds4_gpu_tensor_read(s->graph.logits, 0, s->logits,
                            (uint64_t)DS4_N_VOCAB * sizeof(float)) == 0) {
        payload_set_err(err, errlen, "failed to read Metal logits");
        return 1;
    }
    session_note_logits_valid(s);
    return 0;
...
static bool session_materialize_logits_best_effort(ds4_session *s) {
    char err[128];
    return session_materialize_logits(s, err, sizeof(err)) == 0;
}
...
int ds4_session_save_payload(ds4_session *s, FILE *fp, char *err, size_t errlen) {
    ...
    if (session_materialize_logits(s, err, errlen) != 0) return 1;
    if (ds4_session_is_cpu(s)) {
        const uint32_t raw_live = session_cpu_raw_live_rows(s);
        ...
        if (prompt_text[0] != '\0') {
            ...
            if (prompt.len > 0) {
                if (!metal_graph_reset_prefill_state(&g)) {
                    ...
                    ok = qwen_graph_prefill_imatrix(&g,
                                                    model,
                                                    weights,
                                                    &prompt,
                                                    (uint32_t)prompt.len,
...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    /* Normalize steering vectors: if the array is populated, use only the array.
     * Otherwise fall back to legacy single-vector fields. */
    int nsv = 0;
    if (opt->n_steering_vectors > 0) {
        for (int i = 0; i < opt->n_steering_vectors && nsv < DS4_MAX_STEERING_VECTORS; i++) {
            const ds4_steering_vector *sv = &opt->steering_vectors[i];
            if (!sv->file || !sv->file[0]) continue;
            e->steering_files[nsv] = ds4_strdup(sv->file);
            e->steering_attn_scales[nsv] = sv->attn_scale;
            e->steering_ffn_scales[nsv] = sv->ffn_scale;
            nsv++;
        }
    } else if (opt->directional_steering_file && opt->directional_steering_file[0]) {
        e->steering_files[nsv] = ds4_strdup(opt->directional_steering_file);
        e->steering_attn_scales[nsv] = opt->directional_steering_attn;
        e->steering_ffn_scales[nsv] = opt->directional_steering_ffn;
        nsv++;
    }
    e->n_steering_vectors = nsv;
    for (int i = 0; i < nsv; i++) {
        if (e->steering_attn_scales[i] == 0.0f && e->steering_ffn_scales[i] == 0.0f) continue;
        if (!e->steering_files[i] || !e->steering_files[i][0]) {
            fprintf(stderr, "ds4: directional steering vector %d needs a file\n", i);
            free(e);
            *out = NULL;
            return 1;
    ...
    /* Keep legacy fields pointing to index 0 for backward compat */
    if (nsv > 0) {
        e->directional_steering_file = e->steering_files[0];
        e->directional_steering_attn_scale = e->steering_attn_scales[0];
        e->directional_steering_ffn_scale = e->steering_ffn_scales[0];
    }
    ...
    if (opt->mtp_path && opt->mtp_path[0]) {
        ...
        eagle3_weights_bind(&e->eagle3_weights, &e->mtp_model);
        e->mtp_ready = true;
        fprintf(stderr, "ds4: Qwen3-Coder EAGLE3 support model loaded: %s (draft=%d)\n",
                opt->mtp_path,
                e->mtp_draft_tokens);
    }
...
int ds4_session_create(ds4_session **out, ds4_engine *e, int ctx_size) {
    ...
    s->graph.quality = session_effective_quality(s);
    s->graph.power_percent = (uint32_t)e->power_percent;
    s->graph.eagle3_model = e->mtp_ready ? &e->mtp_model : NULL;
    s->graph.eagle3_weights = e->mtp_ready ? &e->eagle3_weights : NULL;
...
int ds4_session_set_cfg(ds4_session *s, float scale, const ds4_tokens *uncond_tokens) {
    ...
    if (s->quality_override_valid &&
        ds4_session_set_quality(cfg, s->quality_override) != 0) {
...
#define DS4_SESSION_SYNC_RETURN(rc) do { \
        ...
        if (n > 0) {
            ...
            if (n <= decode_suffix_limit) {
                for (uint32_t i = 0; i < n; i++) {
                    ...
                    if (e->mtp_ready && s->graph.eagle3_pending_valid && pos > 0 &&
                        !metal_graph_eval_eagle3_decode(
                                &s->graph, &e->model, &e->weights,
                                s->graph.eagle3_pending_g, s->graph.mtp_state_hc,
                                prompt->v[pos], pos - 1u, false, NULL)) {
                        snprintf(err, errlen, "%s EAGLE3 checkpoint extension failed", backend_name);
                        s->checkpoint_valid = false;
                        DS4_SESSION_SYNC_RETURN(1);
                    }
    ...
    if (!metal_graph_alloc_raw_cap(&s->graph, &e->model, &e->weights, &e->weights.layer[0],
                                   raw_cap, (uint32_t)s->ctx_size, s->prefill_cap, e->mtp_ready) ||
...
int ds4_session_argmax(ds4_session *s) {
    if (!s) return -1;
    if (!s->logits_valid && s->cached_argmax_valid && !s->logit_bias) {
...
int ds4_session_generate_tokens(ds4_session *s, const ds4_generation_options *opt,
                                int *out, char *err, size_t errlen) {
    if (!s || !opt || !out || opt->n_predict < 0 || opt->out_cap < 0) {
        if (errlen) snprintf(err, errlen, "invalid generation arguments");
        return -1;
    }
    int limit = opt->n_predict < opt->out_cap ? opt->n_predict : opt->out_cap;
    if (limit <= 0) return 0;

    uint64_t rng = opt->seed ? opt->seed :
        (((uint64_t)time(NULL) << 32) ^ (uint64_t)(uintptr_t)s);
    int n = 0;
    while (n < limit && ds4_session_pos(s) < ds4_session_ctx(s)) {
        int token = ds4_session_sample(s, opt->temperature, opt->top_k,
                                       opt->top_p, opt->min_p, &rng);
        if (token < 0) {
            if (errlen) snprintf(err, errlen, "failed to sample next token");
            return -1;
        }
        if (token == opt->eos_token) break;
        out[n++] = token;
...
int ds4_session_top_logprobs(ds4_session *s, ds4_token_score *out, int k) {
    ...
    if (!session_materialize_logits_best_effort(s)) return 0;
...
#ifndef DS4_NO_GPU
static void ds4_session_read_metal_capture_after_eval(ds4_session *s) {
    if (!s || ds4_session_is_cpu(s) ||
        !s->capture_enabled ||
        s->capture_buf.n_tokens >= s->capture_buf.capacity) {
        return;
    }
    ds4_gpu_graph *g = &s->graph;
    for (uint32_t ci = 0; ci < g->capture_n_layers; ci++) {
...
static int ds4_session_eval_internal(ds4_session *s, int token, bool probe_mtp,
    ...
    if (!s) return 1;
    if (ds4_session_is_cpu(s)) {
        if (!s->checkpoint_valid || s->checkpoint.len + 1 >= s->ctx_size) {
    ...
    int draft_token = -1;
    if (probe_mtp && e->mtp_ready && s->graph.eagle3_pending_valid &&
        s->checkpoint.len > 0) {
        draft_valid = metal_graph_eval_eagle3_decode(
                &s->graph,
                &e->model,
                &e->weights,
                s->graph.eagle3_pending_g,
                s->graph.mtp_state_hc,
                token,
                (uint32_t)s->checkpoint.len - 1u,
                true,
                &draft_token);
    }
    ...
    g_lora_engine = prev_lora_engine;
    g_lora_scale = prev_lora_scale;
    ds4_session_read_metal_capture_after_eval(s);
    /* Classifier-Free Guidance */
    if (s->cfg_session && s->cfg_scale != 0.0f) {
        if (ds4_session_eval_internal(s->cfg_session, token, false, err, errlen) != 0) {
            s->checkpoint_valid = false;
    ...
    s->checkpoint_valid = true;
    s->mtp_draft_token = draft_token;
    s->mtp_draft_valid = draft_valid;
    if (draft_valid) {
        s->mtp_probe_total++;
        if (sample_argmax((const float *)ds4_gpu_tensor_contents(s->graph.logits),
                          DS4_N_VOCAB) == draft_token) {
            s->mtp_probe_hit++;
        }
    }
    if (need_host_logits) {
...
int ds4_session_eval_argmax(ds4_session *s, int token, char *err, size_t errlen) {
...
#else
    ds4_engine *e = s->engine;
    if (!e || !s->checkpoint_valid || s->checkpoint.len + 1 >= s->ctx_size) {
        snprintf(err, errlen, "session has no valid checkpoint or exceeds context");
        return -1;
    }
    ...
    int top = -1;
    const bool ok = qwen_graph_eval_token_top1(&s->graph, &e->model, &e->weights,
                                               token, (uint32_t)s->checkpoint.len, &top);
    g_lora_engine = prev_lora_engine;
    g_lora_scale = prev_lora_scale;
    if (!ok) {
        snprintf(err, errlen, "%s Qwen top-1 decode failed", ds4_backend_name(e->backend));
        s->checkpoint_valid = false;
        session_note_logits_invalid(s);
        return -1;
    }
    ...
    s->checkpoint_valid = true;
    s->mtp_draft_valid = false;
    session_note_logits_stale_top1(s, top);
    return top;
#endif
}
...
int ds4_session_eval_speculative_argmax(ds4_session *s, int first_token,
     ...
     */
    if (ds4_session_argmax(s) != drafts[0]) {
...
#define DS4_MTP_KEEP_ACCEPTED(n_) do { \
    ...
    for (; draft_n < draft_cap; draft_n++) {
        ...
        if (!metal_graph_eval_mtp_draft_from_hc(&s->graph,
                                                ...
                                                out_hc,
                                                drafts[draft_n - 1],
                                                (uint32_t)(s->checkpoint.len + draft_n - 2),
    ...
    if (use_decode2_exact) {
        ...
        if (ok) {
            ok = qwen_graph_eval_token(&s->graph,
                                       &e->model,
                                       &e->weights,
                                       drafts[0],
                                       (uint32_t)start,
                                       row0_logits);
            if (ok) row0_top = sample_argmax(row0_logits, DS4_N_VOCAB);
            if (ok) {
                ok = qwen_graph_eval_token(&s->graph,
                                           &e->model,
                                           &e->weights,
        ...
        if (ok) {
            ...
            if (commit_drafts == draft_n) {
                ok = metal_graph_read_spec_logits_row(&s->graph,
                                                      ...
                                                      row_logits);
                if (ok) {
                    ok = metal_graph_eagle3_set_pending_batch_row(
                            &s->graph, (uint32_t)(draft_n - 1));
                }
                if (ok) {
                    memcpy(s->logits, row_logits, (size_t)DS4_N_VOCAB * sizeof(s->logits[0]));
            ...
            if (draft_n == 2 && commit_drafts == 1 && capture_prefix1) {
                ...
                if (ok) ok = metal_graph_eagle3_set_pending_batch_row(&s->graph, 0);
                if (ok) {
                    memcpy(s->logits, row_logits, (size_t)DS4_N_VOCAB * sizeof(s->logits[0]));
                    accepted[n_accept++] = drafts[0];
                    s->checkpoint_valid = true;
            ...
            if (ok) {
                /*
                 * A partial accept must continue from the exact one-token
                 * Qwen graph. Tiny batch reductions can perturb close logits;
                 * replaying the accepted prefix also refreshes EAGLE's pending
                 * target feature with the committed target state.
                 */
                for (int i = 0; ok && i < commit_drafts; i++) {
                    ok = qwen_graph_eval_token(&s->graph,
                                               &e->model,
                                               &e->weights,
                                               drafts[i],
                                               (uint32_t)s->checkpoint.len,
                                               s->logits);
                    if (ok) token_vec_push(&s->checkpoint, drafts[i]);
                }
                if (ok) {
    ...
    for (int i = 0; i < draft_n && n_accept < accepted_cap; i++) {
        ...
        if (!qwen_graph_eval_token(&s->graph,
                                   &e->model,
                                   &e->weights,
                                   drafts[i],
                                   (uint32_t)s->checkpoint.len,
...
void ds4_session_rewind(ds4_session *s, int pos) {
    ...
    session_note_logits_invalid(s);
    if (ds4_session_is_cpu(s)) {
        /* Rewinding a sliding-window / compression cache to an arbitrary point
         * is not supported without a full snapshot.  Invalidate so the next sync
         * re-prefills from the rewound prefix. */
        kv_cache_free(&s->cpu_cache);
        kv_cache_init(&s->cpu_cache, (uint32_t)s->ctx_size, 0);
        s->checkpoint_valid = false;
    }
