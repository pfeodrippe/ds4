...
static id<MTLComputePipelineState> g_moe_mul_mv_id_q4_k_sum8_pipeline;
static id<MTLComputePipelineState> g_moe_mul_mv_id_q6_k_pipeline;
static id<MTLComputePipelineState> g_moe_mul_mv_id_q6_k_sum8_pipeline;
...
static id<MTLComputePipelineState> ds4_gpu_get_mul_mv_pipeline(const char *function_name,
                                                               int16_t nsg);
...
static int ds4_gpu_map_model_views(
    ...
    const int warm_model_views = getenv("DS4_METAL_MODEL_WARMUP") != NULL;
    if (warm_model_views) {
        /*
         * Optional diagnostic warmup. It can create extra VM pressure on 48 GB
         * machines, so normal startup leaves first-touch costs to real graph
         * execution instead of making warmup a correctness gate.
         */
    ...
    if (!warmed) {
        fprintf(stderr, "ds4: Metal model warmup failed; continuing without warmup\n");
    }
    return 1;
}
...
static id<MTLComputePipelineState> ds4_gpu_static_pipeline(
        id<MTLComputePipelineState> __strong *slot,
        const char                         *function_name) {
    if (!*slot) *slot = ds4_gpu_get_pipeline(function_name);
    return *slot;
}

static id<MTLComputePipelineState> ds4_gpu_static_mul_mv_pipeline(
        id<MTLComputePipelineState> __strong *slot,
        const char                         *function_name,
        int16_t                             nsg) {
    if (!*slot) *slot = ds4_gpu_get_mul_mv_pipeline(function_name, nsg);
    return *slot;
}
...
static id<MTLComputePipelineState> ds4_gpu_qwen_k_mv_pipeline(uint32_t tensor_type) {
    ...
    if (tensor_type == DS4_METAL_TENSOR_Q4_K) {
        return ds4_gpu_static_mul_mv_pipeline(&q4_pipeline,
...
static id<MTLComputePipelineState> ds4_gpu_qwen_router_mv_pipeline(void) {
    static id<MTLComputePipelineState> pipeline;
    return ds4_gpu_static_mul_mv_pipeline(&pipeline,
                                          "kernel_mul_mv_f32_f32_4",
                                          8);
}

static id<MTLComputePipelineState> ds4_gpu_qwen_norm_rope_pipeline(void) {
    static id<MTLComputePipelineState> pipeline;
    return ds4_gpu_static_pipeline(&pipeline,
                                   "kernel_qwen_norm_rope_weight_f32");
}

static id<MTLComputePipelineState> ds4_gpu_qwen_norm_rope_store_pipeline(void) {
    static id<MTLComputePipelineState> pipeline;
    return ds4_gpu_static_pipeline(&pipeline,
                                   "kernel_qwen_norm_rope_store_kv_f32");
}

static id<MTLComputePipelineState> ds4_gpu_qwen_norm_rope_qkv_store_pipeline(void) {
    static id<MTLComputePipelineState> pipeline;
    return ds4_gpu_static_pipeline(&pipeline,
                                   "kernel_qwen_norm_rope_weight_store_kv_f32");
}

static id<MTLComputePipelineState> ds4_gpu_qwen_router_select_pipeline(void) {
    static id<MTLComputePipelineState> pipeline;
    return ds4_gpu_static_pipeline(&pipeline,
...
static int ds4_gpu_cached_env_present(
        const char *name,
        int        *initialized,
        int        *enabled) {
    if (!*initialized) {
        *enabled = getenv(name) != NULL;
        *initialized = 1;
    }
    return *enabled;
}

static int ds4_gpu_moe_write_clamped_act(void) {
    static int initialized;
    static int enabled;
    return ds4_gpu_cached_env_present("DS4_METAL_MOE_WRITE_CLAMPED_ACT",
                                      &initialized,
                                      &enabled);
}

static int ds4_gpu_disable_routed_pair_swiglu_fusion(void) {
    static int initialized;
    static int enabled;
    return ds4_gpu_cached_env_present("DS4_METAL_DISABLE_ROUTED_PAIR_SWIGLU_FUSION",
                                      &initialized,
                                      &enabled);
}

static int ds4_gpu_moe_stage_profile_enabled(void) {
    static int initialized;
    static int enabled;
...
static int __attribute__((unused)) ds4_gpu_compile_tensor_probe(void) {
...
static id<MTLComputePipelineState> ds4_gpu_get_mul_mv_pipeline(
    ...
    int16_t one = 1;
    [constants setConstantValue:&one type:MTLDataTypeShort atIndex:602];
    [constants setConstantValue:&one type:MTLDataTypeShort atIndex:603];
    [constants setConstantValue:&one type:MTLDataTypeShort atIndex:604];
...
static ds4_gpu_q8_0_matvec_args ds4_gpu_make_k_mv_args(
        uint64_t in_dim,
        uint64_t out_dim,
        uint64_t row_bytes,
        int32_t  nr0) {
...
typedef struct {
    ...
    uint32_t row_bytes;
    uint32_t n_tok;
} ds4_gpu_qwen_matmul_args;

typedef struct {
    uint32_t width;
} ds4_gpu_qwen_row_args;

typedef struct {
    uint32_t n_head;
    uint32_t head_dim;
    float    eps;
} ds4_gpu_qwen_head_norm_args;
...
typedef struct {
    ...
    float    freq_base;
    float    eps;
} ds4_gpu_qwen_norm_rope_store_args;
...
typedef struct {
    ...
    float    freq_base;
    float    eps;
} ds4_gpu_qwen_norm_rope_qkv_store_args;
...
typedef struct {
    ...
    uint32_t kv_dim;
    uint32_t head_dim;
} ds4_gpu_qwen_store_kv_batch_args;
...
typedef struct {
    ...
    uint32_t head_dim;
    float    scale;
} ds4_gpu_qwen_attention_batch_args;

typedef struct {
    uint32_t n_expert;
    uint32_t n_expert_used;
} ds4_gpu_qwen_router_args;
...
#pragma clang diagnostic pop
        ...
        int16_t moe_mv_id_nsg = DS4_MOE_MV_ID_NSG;
        int16_t moe_mv_one = 1;
        [moe_mv_id_constants setConstantValue:&moe_mv_id_nsg type:MTLDataTypeShort atIndex:600];
        [moe_mv_id_constants setConstantValue:&moe_mv_one type:MTLDataTypeShort atIndex:602];
        [moe_mv_id_constants setConstantValue:&moe_mv_one type:MTLDataTypeShort atIndex:603];
        [moe_mv_id_constants setConstantValue:&moe_mv_one type:MTLDataTypeShort atIndex:604];
        ...
        error = nil;
        fn = [library newFunctionWithName:@"kernel_mul_mv_id_q4_K_sum8_f32"
                           constantValues:moe_mv_id_constants
                                    error:&error];
        if (!fn) {
            fprintf(stderr, "ds4: Metal kernel_mul_mv_id_q4_K_sum8_f32 function not found: %s\n",
                    [[error localizedDescription] UTF8String]);
            g_queue = nil;
            g_device = nil;
            return 0;
        }
        g_moe_mul_mv_id_q4_k_sum8_pipeline = [g_device newComputePipelineStateWithFunction:fn error:&error];
        if (!g_moe_mul_mv_id_q4_k_sum8_pipeline) {
            fprintf(stderr, "ds4: Metal kernel_mul_mv_id_q4_K_sum8_f32 pipeline failed: %s\n",
                    [[error localizedDescription] UTF8String]);
            g_queue = nil;
            g_device = nil;
            return 0;
        ...
        error = nil;
        fn = [library newFunctionWithName:@"kernel_mul_mv_id_q6_K_f32"
                           constantValues:moe_mv_id_constants
                                    error:&error];
        if (!fn) {
            fprintf(stderr, "ds4: Metal kernel_mul_mv_id_q6_K_f32 function not found: %s\n",
                    [[error localizedDescription] UTF8String]);
            g_queue = nil;
            g_device = nil;
            return 0;
        }
        g_moe_mul_mv_id_q6_k_pipeline = [g_device newComputePipelineStateWithFunction:fn error:&error];
        if (!g_moe_mul_mv_id_q6_k_pipeline) {
            fprintf(stderr, "ds4: Metal kernel_mul_mv_id_q6_K_f32 pipeline failed: %s\n",
                    [[error localizedDescription] UTF8String]);
            g_queue = nil;
            g_device = nil;
            return 0;
        ...
        error = nil;
        fn = [library newFunctionWithName:@"kernel_mul_mv_id_q6_K_sum8_f32"
                           constantValues:moe_mv_id_constants
                                    error:&error];
        if (!fn) {
            fprintf(stderr, "ds4: Metal kernel_mul_mv_id_q6_K_sum8_f32 function not found: %s\n",
                    [[error localizedDescription] UTF8String]);
            g_queue = nil;
            g_device = nil;
            return 0;
        }
        g_moe_mul_mv_id_q6_k_sum8_pipeline = [g_device newComputePipelineStateWithFunction:fn error:&error];
        if (!g_moe_mul_mv_id_q6_k_sum8_pipeline) {
            fprintf(stderr, "ds4: Metal kernel_mul_mv_id_q6_K_sum8_f32 pipeline failed: %s\n",
                    [[error localizedDescription] UTF8String]);
            g_queue = nil;
            g_device = nil;
            return 0;
...
ds4_gpu_tensor *ds4_gpu_tensor_alloc_private(uint64_t bytes) {
    ...
    @autoreleasepool {
        ...
        if (ds4_gpu_trace_allocs()) {
            fprintf(stderr,
                    "ds4: Metal private tensor alloc %.3f MiB live %.3f MiB peak %.3f MiB\n",
                    (double)bytes / (1024.0 * 1024.0),
                    (double)g_tensor_alloc_live_bytes / (1024.0 * 1024.0),
                    (double)g_tensor_alloc_peak_bytes / (1024.0 * 1024.0));
        }
...
int ds4_gpu_tensor_write(ds4_gpu_tensor *tensor, uint64_t offset, const void *data, uint64_t bytes) {
    ...
    if (bytes != 0) {
        if ([obj.buffer storageMode] == MTLStorageModePrivate) {
            id<MTLBuffer> staging = [g_device newBufferWithBytes:data
            ...
            [blit copyFromBuffer:staging
                    sourceOffset:0
                        toBuffer:obj.buffer
               destinationOffset:(NSUInteger)(obj.offset + offset)
                            size:(NSUInteger)bytes];
            [blit endEncoding];
            [cb commit];
            if (!ds4_gpu_wait_command_buffer(cb, "private tensor write")) return 0;
        } else {
...
int ds4_gpu_tensor_read(const ds4_gpu_tensor *tensor, uint64_t offset, void *data, uint64_t bytes) {
    ...
    if (bytes != 0) {
        if ([obj.buffer storageMode] == MTLStorageModePrivate) {
            id<MTLBuffer> staging = [g_device newBufferWithLength:(NSUInteger)bytes
                                                          options:MTLResourceStorageModeShared];
            if (!staging) return 0;
            id<MTLCommandBuffer> cb = [g_queue commandBuffer];
            id<MTLBlitCommandEncoder> blit = [cb blitCommandEncoder];
            if (!blit) return 0;
            [blit copyFromBuffer:obj.buffer
                    sourceOffset:(NSUInteger)(obj.offset + offset)
                        toBuffer:staging
               destinationOffset:0
                            size:(NSUInteger)bytes];
            [blit endEncoding];
            [cb commit];
            if (!ds4_gpu_wait_command_buffer(cb, "private tensor read")) return 0;
            memcpy(data, [staging contents], (size_t)bytes);
        } else {
...
void ds4_gpu_cleanup(void) {
    ...
    @autoreleasepool {
        ...
        g_moe_mul_mv_id_q4_k_sum8_pipeline = nil;
        g_moe_mul_mv_id_q6_k_pipeline = nil;
        g_moe_mul_mv_id_q6_k_sum8_pipeline = nil;
...
int ds4_gpu_qwen_router_cached_tensor(
    ...
    if (!out || !router_weights || !x ||
        ...
        weight_bytes > ds4_gpu_tensor_bytes(router_weights) - weight_offset ||
        ds4_gpu_tensor_bytes(x) < (uint64_t)QWEN_ROUTER_IN_DIM * sizeof(float) ||
        ds4_gpu_tensor_bytes(out) < (uint64_t)QWEN_ROUTER_OUT_DIM * sizeof(float)) {
        return 0;
    }

    @autoreleasepool {
        id<MTLBuffer> wbuf = ds4_gpu_tensor_buffer(router_weights);
        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        id<MTLBuffer> outbuf = ds4_gpu_tensor_buffer(out);
        ...
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        [enc setBuffer:wbuf
                offset:(NSUInteger)(ds4_gpu_tensor_offset(router_weights) + weight_offset)
               atIndex:1];
        [enc setBuffer:xbuf offset:ds4_gpu_tensor_offset(x) atIndex:2];
        [enc setBuffer:outbuf offset:ds4_gpu_tensor_offset(out) atIndex:3];
        [enc setThreadgroupMemoryLength:dispatch.smem atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake(
                (QWEN_ROUTER_OUT_DIM + (NSUInteger)dispatch.nr0 - 1u) /
                    (NSUInteger)dispatch.nr0,
                1,
                1)
             threadsPerThreadgroup:MTLSizeMake(32, (NSUInteger)dispatch.nsg, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        return ds4_gpu_finish_command_buffer(cb, owned, "Qwen cached router matvec");
    }
}
...
int ds4_gpu_matmul_f32_tensor(
    ...
    @autoreleasepool {
        ...
        if (n_tok > 1) {
            id<MTLComputePipelineState> pipeline = ds4_gpu_get_pipeline("kernel_qwen_matmul_f32_f32");
            if (!pipeline) return 0;
            ...
            [enc dispatchThreadgroups:MTLSizeMake((NSUInteger)out_dim, (NSUInteger)n_tok, 1)
                 threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
            ds4_gpu_end_compute_encoder(cb, enc);
            if (!ds4_gpu_finish_command_buffer(cb, owned, "F32 tensor batch matmul")) return 0;
            return 1;
        }

        const bool qwen_router = !g_quality_mode && in_dim == 2048u && out_dim == 128u;
...
int ds4_gpu_dequant_k_row_tensor(
        ...
        uint32_t                width) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!out || !model_map || width == 0 || (width % 256u) != 0) return 0;
    const uint64_t row_bytes = ds4_gpu_k_row_bytes(tensor_type, width);
    if (row_bytes == 0 || row_offset > model_size || row_bytes > model_size - row_offset) return 0;

    @autoreleasepool {
        id<MTLComputePipelineState> pipeline = nil;
        if (tensor_type == DS4_METAL_TENSOR_Q4_K) {
            pipeline = ds4_gpu_get_pipeline("kernel_qwen_dequant_q4_K_row");
        } else if (tensor_type == DS4_METAL_TENSOR_Q6_K) {
            pipeline = ds4_gpu_get_pipeline("kernel_qwen_dequant_q6_K_row");
        }
        if (!pipeline) return 0;

        id<MTLBuffer> outbuf = ds4_gpu_tensor_buffer(out);
        if (!outbuf || ds4_gpu_tensor_bytes(out) < (uint64_t)width * sizeof(float)) return 0;
        uint64_t inner = 0;
        id<MTLBuffer> wbuf = ds4_gpu_wrap_model_range(model_map, model_size, row_offset, row_bytes, &inner);
        if (!wbuf) return 0;

        int owned = 0;
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        ds4_gpu_qwen_row_args args = { .width = width };
        id<MTLComputeCommandEncoder> enc = ds4_gpu_compute_encoder(cb);
        [enc setComputePipelineState:pipeline];
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        ...
        [enc dispatchThreads:MTLSizeMake(width, 1, 1)
         threadsPerThreadgroup:MTLSizeMake(MIN((NSUInteger)width, (NSUInteger)256), 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen K-row dequant")) return 0;
    }
    return 1;
}

int ds4_gpu_matmul_k_tensor(
        ds4_gpu_tensor       *out,
        const void             *model_map,
        uint64_t                model_size,
        uint64_t                weight_offset,
        ...
        const ds4_gpu_tensor *x) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!out || !x || !model_map || in_dim > UINT32_MAX || out_dim > UINT32_MAX ||
        (in_dim % 256u) != 0) return 0;
    const uint64_t row_bytes = ds4_gpu_k_row_bytes(tensor_type, in_dim);
    const uint64_t weight_bytes = row_bytes * out_dim;
    ...
    @autoreleasepool {
        ...
        if (!xbuf || !outbuf ||
            ...
            ds4_gpu_tensor_bytes(out) < out_dim * sizeof(float)) return 0;
        uint64_t inner = 0;
        ...
        [enc setBuffer:xbuf offset:ds4_gpu_tensor_offset(x) atIndex:2];
        [enc setBuffer:outbuf offset:ds4_gpu_tensor_offset(out) atIndex:3];
        [enc dispatchThreadgroups:MTLSizeMake(((NSUInteger)out_dim + (NSUInteger)(nr0 * nsg) - 1u) /
                                              (NSUInteger)(nr0 * nsg),
                                              1,
                                              1)
             threadsPerThreadgroup:MTLSizeMake(32, (NSUInteger)nsg, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen optimized K matvec")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_qkv_k_tensor(
    ...
    const uint64_t v_bytes = v_row_bytes * QWEN_KV_DIM;
    if (q_weight_offset > model_size || q_bytes > model_size - q_weight_offset ||
        k_weight_offset > model_size || kv_bytes > model_size - k_weight_offset ||
        v_weight_offset > model_size || v_bytes > model_size - v_weight_offset ||
        ds4_gpu_tensor_bytes(x) < (uint64_t)QWEN_IN_DIM * sizeof(float) ||
        ds4_gpu_tensor_bytes(q) < (uint64_t)QWEN_Q_DIM * sizeof(float) ||
        ds4_gpu_tensor_bytes(k) < (uint64_t)QWEN_KV_DIM * sizeof(float) ||
        ds4_gpu_tensor_bytes(v) < (uint64_t)QWEN_KV_DIM * sizeof(float)) {
        return 0;
    }
    ...
    @autoreleasepool {
        ...
        uint64_t v_inner = 0;
        id<MTLBuffer> q_wbuf =
            ds4_gpu_wrap_model_range(model_map, model_size, q_weight_offset, q_bytes, &q_inner);
        id<MTLBuffer> k_wbuf =
            ds4_gpu_wrap_model_range(model_map, model_size, k_weight_offset, kv_bytes, &k_inner);
        id<MTLBuffer> v_wbuf =
            ds4_gpu_wrap_model_range(model_map, model_size, v_weight_offset, v_bytes, &v_inner);
        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        id<MTLBuffer> qbuf = ds4_gpu_tensor_buffer(q);
        ...
        ds4_gpu_q8_0_matvec_args q_args =
            ds4_gpu_make_k_mv_args(QWEN_IN_DIM, QWEN_Q_DIM, row_bytes, q_nr0);
        ds4_gpu_q8_0_matvec_args k_args =
            ds4_gpu_make_k_mv_args(QWEN_IN_DIM, QWEN_KV_DIM, row_bytes, q_nr0);
        ds4_gpu_q8_0_matvec_args v_args =
            ds4_gpu_make_k_mv_args(QWEN_IN_DIM, QWEN_KV_DIM, v_row_bytes, v_nr0);
        const NSUInteger q_rows_per_group = (NSUInteger)(q_nr0 * nsg);
        const NSUInteger v_rows_per_group = (NSUInteger)(v_nr0 * nsg);
        const NSUInteger q_groups =
            (QWEN_Q_DIM + q_rows_per_group - 1u) / q_rows_per_group;
        const NSUInteger k_groups =
            (QWEN_KV_DIM + q_rows_per_group - 1u) / q_rows_per_group;
        const NSUInteger v_groups =
            (QWEN_KV_DIM + v_rows_per_group - 1u) / v_rows_per_group;
        ...
        [enc setBytes:&k_args length:sizeof(k_args) atIndex:1];
        [enc setBytes:&v_args length:sizeof(v_args) atIndex:2];
        [enc setBuffer:q_wbuf offset:(NSUInteger)q_inner atIndex:3];
        [enc setBuffer:k_wbuf offset:(NSUInteger)k_inner atIndex:4];
        [enc setBuffer:v_wbuf offset:(NSUInteger)v_inner atIndex:5];
        [enc setBuffer:xbuf offset:ds4_gpu_tensor_offset(x) atIndex:6];
        [enc setBuffer:qbuf offset:ds4_gpu_tensor_offset(q) atIndex:7];
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k) atIndex:8];
        [enc setBuffer:vbuf offset:ds4_gpu_tensor_offset(v) atIndex:9];
        [enc dispatchThreadgroups:MTLSizeMake(q_groups + k_groups + v_groups, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(32, (NSUInteger)nsg, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        return ds4_gpu_finish_command_buffer(cb, owned, "Qwen fused QKV Q4 matvec");
    }
}
...
int ds4_gpu_matmul_k_argmax_tensor(
        ...
        const ds4_gpu_tensor *x) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!scratch || !top_id || !x || !model_map ||
        tensor_type != DS4_METAL_TENSOR_Q6_K ||
        in_dim > UINT32_MAX || out_dim > UINT32_MAX ||
        (in_dim % 256u) != 0) {
        return 0;
    }
    ...
    @autoreleasepool {
        const int16_t nsg = DS4_QWEN_K_MV_NSG;
        const int32_t nr0 = 8;
        const uint32_t rows_per_group = (uint32_t)(nr0 * nsg);
        const uint32_t n_groups = (uint32_t)((out_dim + rows_per_group - 1u) / rows_per_group);
        const uint64_t scratch_bytes = (uint64_t)n_groups * 2u * sizeof(float);
        if (scratch_bytes > ds4_gpu_tensor_bytes(scratch) ||
            ds4_gpu_tensor_bytes(top_id) < sizeof(int)) {
            return 0;
        }

        id<MTLComputePipelineState> argmax_pipeline =
            ds4_gpu_get_mul_mv_pipeline("kernel_qwen_argmax_q6_K_f32", nsg);
        id<MTLComputePipelineState> reduce_pipeline =
            ds4_gpu_get_pipeline("kernel_qwen_argmax_reduce_f32");
        if (!argmax_pipeline || !reduce_pipeline) return 0;

        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        id<MTLBuffer> scratchbuf = ds4_gpu_tensor_buffer(scratch);
        id<MTLBuffer> topbuf = ds4_gpu_tensor_buffer(top_id);
        if (!xbuf || !scratchbuf || !topbuf ||
            ds4_gpu_tensor_bytes(x) < in_dim * sizeof(float)) {
            return 0;
        }
        uint64_t inner = 0;
        ...
        ds4_gpu_q8_0_matvec_args args = ds4_gpu_make_k_mv_args(in_dim, out_dim, row_bytes, nr0);
        id<MTLComputeCommandEncoder> enc = ds4_gpu_compute_encoder(cb);
        [enc setComputePipelineState:argmax_pipeline];
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        [enc setBuffer:wbuf offset:(NSUInteger)inner atIndex:1];
        [enc setBuffer:xbuf offset:ds4_gpu_tensor_offset(x) atIndex:2];
        [enc setBuffer:scratchbuf offset:ds4_gpu_tensor_offset(scratch) atIndex:3];
        [enc setThreadgroupMemoryLength:(NSUInteger)rows_per_group * 2u * sizeof(float) atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake(n_groups, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(32, (NSUInteger)nsg, 1)];

        const NSUInteger nth = 256u;
        [enc setComputePipelineState:reduce_pipeline];
        [enc setBytes:&n_groups length:sizeof(n_groups) atIndex:0];
        [enc setBuffer:scratchbuf offset:ds4_gpu_tensor_offset(scratch) atIndex:1];
        [enc setBuffer:topbuf offset:ds4_gpu_tensor_offset(top_id) atIndex:2];
        [enc setThreadgroupMemoryLength:nth * 2u * sizeof(float) atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake(1, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);

        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen K argmax matvec")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_head_rms_norm_weight_tensor(
        ds4_gpu_tensor       *x,
        const void             *model_map,
        uint64_t                model_size,
        uint64_t                weight_offset,
        ...
        float                   eps) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!x || !model_map || n_head == 0 || head_dim == 0 || (head_dim & 3u) != 0) return 0;
    const uint64_t x_bytes = (uint64_t)n_head * head_dim * sizeof(float);
    ...
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline =
            ds4_gpu_get_pipeline("kernel_qwen_head_rms_norm_weight_f32");
        if (!pipeline) return 0;
        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        if (!xbuf || ds4_gpu_tensor_bytes(x) < x_bytes) return 0;
        uint64_t inner = 0;
        id<MTLBuffer> wbuf = ds4_gpu_wrap_model_range(model_map, model_size, weight_offset, w_bytes, &inner);
        if (!wbuf) return 0;

        int owned = 0;
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        NSUInteger nth = head_dim < 256u ? head_dim : 256u;
        ds4_gpu_qwen_head_norm_args args = {
            .n_head = n_head,
            .head_dim = head_dim,
            .eps = eps,
        };
        ...
        [enc dispatchThreadgroups:MTLSizeMake(n_head, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen head RMS norm")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_norm_rope_weight_tensor(
        ...
        float                   eps) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!x || !model_map || n_head == 0 || head_dim == 0 || (head_dim & 1u) != 0) return 0;
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline =
            ds4_gpu_qwen_norm_rope_pipeline();
        if (!pipeline) return 0;
        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        ...
        uint64_t inner = 0;
        id<MTLBuffer> wbuf = ds4_gpu_wrap_model_range(model_map, model_size, weight_offset, bytes, &inner);
        if (!wbuf) return 0;
        int owned = 0;
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        NSUInteger nth = head_dim < 256u ? head_dim : 256u;
        ds4_gpu_qwen_norm_rope_args args = {
            .n_head = n_head,
            .head_dim = head_dim,
            .pos = pos,
            .freq_base = freq_base,
        ...
        [enc dispatchThreadgroups:MTLSizeMake(n_head, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen norm RoPE")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_norm_rope_store_kv_tensor(
        ...
        float                   eps) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!k_cache || !v_cache || !k || !v || !model_map ||
        n_head == 0 || head_dim == 0 || (head_dim & 1u) != 0 || row >= cap) {
        return 0;
    }
    ...
    @autoreleasepool {
        ...
        if (!pipeline) return 0;
        id<MTLBuffer> kcbuf = ds4_gpu_tensor_buffer(k_cache);
        id<MTLBuffer> vcbuf = ds4_gpu_tensor_buffer(v_cache);
        id<MTLBuffer> kbuf = ds4_gpu_tensor_buffer(k);
        id<MTLBuffer> vbuf = ds4_gpu_tensor_buffer(v);
        if (!kcbuf || !vcbuf || !kbuf || !vbuf ||
            ds4_gpu_tensor_bytes(k_cache) < cache_bytes ||
            ds4_gpu_tensor_bytes(v_cache) < cache_bytes ||
            ds4_gpu_tensor_bytes(k) < row_bytes ||
            ds4_gpu_tensor_bytes(v) < row_bytes ||
            weight_offset > model_size || head_dim * sizeof(float) > model_size - weight_offset) {
            return 0;
        }
        uint64_t inner = 0;
        ...
        if (!cb) return 0;
        NSUInteger nth = head_dim < 256u ? head_dim : 256u;
        ds4_gpu_qwen_norm_rope_store_args args = {
            .row = row,
            .cap = cap,
            .n_head = n_head,
            .head_dim = head_dim,
        ...
        [enc setComputePipelineState:pipeline];
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        [enc setBuffer:kcbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:1];
        [enc setBuffer:vcbuf offset:ds4_gpu_tensor_offset(v_cache) atIndex:2];
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k) atIndex:3];
        [enc setBuffer:vbuf offset:ds4_gpu_tensor_offset(v) atIndex:4];
        [enc setBuffer:wbuf offset:(NSUInteger)inner atIndex:5];
        [enc setThreadgroupMemoryLength:nth * sizeof(float) atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake(n_head, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen norm RoPE KV store")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_norm_rope_weight_store_kv_tensor(
        ...
        float                   eps) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!q || !k_cache || !v_cache || !k || !v || !rope_cos || !rope_sin || !model_map ||
        n_q_head == 0 || n_kv_head == 0 || head_dim == 0 ||
        (head_dim & 1u) != 0 || row >= cap) {
        return 0;
    }
    const uint64_t q_dim = (uint64_t)n_q_head * head_dim;
    ...
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline =
            ds4_gpu_qwen_norm_rope_qkv_store_pipeline();
        if (!pipeline) return 0;
        id<MTLBuffer> qbuf = ds4_gpu_tensor_buffer(q);
        id<MTLBuffer> kcbuf = ds4_gpu_tensor_buffer(k_cache);
        id<MTLBuffer> vcbuf = ds4_gpu_tensor_buffer(v_cache);
        id<MTLBuffer> kbuf = ds4_gpu_tensor_buffer(k);
        id<MTLBuffer> vbuf = ds4_gpu_tensor_buffer(v);
        id<MTLBuffer> cbuf = ds4_gpu_tensor_buffer(rope_cos);
        id<MTLBuffer> sbuf = ds4_gpu_tensor_buffer(rope_sin);
        if (!qbuf || !kcbuf || !vcbuf || !kbuf || !vbuf || !cbuf || !sbuf ||
            ds4_gpu_tensor_bytes(q) < q_bytes ||
            ds4_gpu_tensor_bytes(k_cache) < cache_bytes ||
            ds4_gpu_tensor_bytes(v_cache) < cache_bytes ||
            ds4_gpu_tensor_bytes(k) < kv_row_bytes ||
            ds4_gpu_tensor_bytes(v) < kv_row_bytes ||
            ds4_gpu_tensor_bytes(rope_cos) < ((uint64_t)row + 1u) * (uint64_t)(head_dim / 2u) * sizeof(float) ||
            ds4_gpu_tensor_bytes(rope_sin) < ((uint64_t)row + 1u) * (uint64_t)(head_dim / 2u) * sizeof(float) ||
        ...
        uint64_t q_inner = 0;
        uint64_t k_inner = 0;
        id<MTLBuffer> q_wbuf = ds4_gpu_wrap_model_range(model_map,
                                                         model_size,
                                                         q_weight_offset,
                                                         weight_bytes,
                                                         &q_inner);
        id<MTLBuffer> k_wbuf = ds4_gpu_wrap_model_range(model_map,
                                                         model_size,
                                                         k_weight_offset,
                                                         weight_bytes,
                                                         &k_inner);
        ...
        if (!cb) return 0;
        NSUInteger nth = head_dim < 256u ? head_dim : 256u;
        ds4_gpu_qwen_norm_rope_qkv_store_args args = {
            .row = row,
            .cap = cap,
            .n_q_head = n_q_head,
            .n_kv_head = n_kv_head,
        ...
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        [enc setBuffer:qbuf offset:ds4_gpu_tensor_offset(q) atIndex:1];
        [enc setBuffer:kcbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:2];
        [enc setBuffer:vcbuf offset:ds4_gpu_tensor_offset(v_cache) atIndex:3];
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k) atIndex:4];
        [enc setBuffer:vbuf offset:ds4_gpu_tensor_offset(v) atIndex:5];
        [enc setBuffer:q_wbuf offset:(NSUInteger)q_inner atIndex:6];
        [enc setBuffer:k_wbuf offset:(NSUInteger)k_inner atIndex:7];
        const NSUInteger rope_off = (NSUInteger)((uint64_t)row * (uint64_t)(head_dim / 2u) * sizeof(float));
        [enc setBuffer:cbuf offset:ds4_gpu_tensor_offset(rope_cos) + rope_off atIndex:8];
        [enc setBuffer:sbuf offset:ds4_gpu_tensor_offset(rope_sin) + rope_off atIndex:9];
        [enc setThreadgroupMemoryLength:nth * sizeof(float) atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake((NSUInteger)n_q_head + (NSUInteger)n_kv_head, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen fused norm RoPE KV store")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_store_kv_tensor(
        ...
        uint32_t                head_dim) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!k_cache || !v_cache || !k || !v || kv_dim == 0 || head_dim == 0 ||
        kv_dim % head_dim != 0 || cap == 0 || row >= cap) return 0;
    const uint64_t cache_bytes = (uint64_t)cap * kv_dim * sizeof(uint16_t);
    ...
    @autoreleasepool {
        ...
        if (!kcbuf || !vcbuf || !kbuf || !vbuf ||
            ...
            ds4_gpu_tensor_bytes(v_cache) < cache_bytes ||
            ds4_gpu_tensor_bytes(k) < row_bytes ||
            ds4_gpu_tensor_bytes(v) < row_bytes) return 0;
        int owned = 0;
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        ds4_gpu_qwen_store_kv_args args = {
            .row = row,
            .cap = cap,
            .kv_dim = kv_dim,
            .head_dim = head_dim,
        ...
        [enc dispatchThreads:MTLSizeMake(kv_dim, 1, 1)
         threadsPerThreadgroup:MTLSizeMake(MIN((NSUInteger)kv_dim, (NSUInteger)256), 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen KV store")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_store_kv_batch_tensor(
    ...
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!k_cache || !v_cache || !k || !v || kv_dim == 0 || head_dim == 0 ||
        kv_dim % head_dim != 0 || cap == 0 || n_tokens == 0 ||
    ...
    @autoreleasepool {
        ...
        if (!kcbuf || !vcbuf || !kbuf || !vbuf ||
            ds4_gpu_tensor_bytes(k_cache) < cache_bytes ||
            ds4_gpu_tensor_bytes(v_cache) < cache_bytes ||
            ds4_gpu_tensor_bytes(k) < batch_bytes ||
            ds4_gpu_tensor_bytes(v) < batch_bytes) return 0;
        int owned = 0;
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        ds4_gpu_qwen_store_kv_batch_args args = {
            .pos0 = pos0,
            .n_tokens = n_tokens,
            .cap = cap,
            .kv_dim = kv_dim,
        ...
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k) atIndex:3];
        [enc setBuffer:vbuf offset:ds4_gpu_tensor_offset(v) atIndex:4];
        const uint64_t total = (uint64_t)n_tokens * kv_dim;
        [enc dispatchThreads:MTLSizeMake((NSUInteger)total, 1, 1)
         threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen batch KV store")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_attention_tensor(
    ...
    @autoreleasepool {
        ...
        if (head_dim != QWEN_HEAD_DIM || n_head % n_head_kv != 0) return 0;

        id<MTLBuffer> hbuf = ds4_gpu_tensor_buffer(heads);
        id<MTLBuffer> qbuf = ds4_gpu_tensor_buffer(q);
        id<MTLBuffer> kbuf = ds4_gpu_tensor_buffer(k_cache);
        id<MTLBuffer> vbuf = ds4_gpu_tensor_buffer(v_cache);
        if (!hbuf || !qbuf || !kbuf || !vbuf ||
            ds4_gpu_tensor_bytes(heads) < q_bytes ||
            ds4_gpu_tensor_bytes(q) < q_bytes ||
            ds4_gpu_tensor_bytes(k_cache) < kv_bytes ||
            ds4_gpu_tensor_bytes(v_cache) < kv_bytes) return 0;

        const bool has_kvpad = (n_ctx % QWEN_FLASH_NCPSG) != 0;
        const uint32_t nsg =
            ds4_gpu_flash_attn_vec_nsg(n_ctx, QWEN_FLASH_NWG, QWEN_FLASH_NCPSG);
        id<MTLComputePipelineState> pad_pipeline = has_kvpad ?
            ds4_gpu_get_flash_attn_pad_pipeline(false, QWEN_FLASH_NCPSG) : nil;
        id<MTLComputePipelineState> flash_pipeline =
            ds4_gpu_get_flash_attn_vec_pipeline(
                    "kernel_flash_attn_ext_vec_f16_dk128_dv128",
                    false, false, false, false, has_kvpad,
                    head_dim, head_dim, (int32_t)nsg, QWEN_FLASH_NWG);
        id<MTLComputePipelineState> reduce_pipeline =
            ds4_gpu_get_flash_attn_reduce_pipeline(head_dim, QWEN_FLASH_NWG);
        if ((has_kvpad && !pad_pipeline) || !flash_pipeline || !reduce_pipeline) return 0;

        const NSUInteger q_row_bytes = (NSUInteger)head_dim * sizeof(float);
        const NSUInteger kv_row_bytes = (NSUInteger)head_dim * sizeof(uint16_t);
        const NSUInteger pad_bytes =
            2u * QWEN_FLASH_NCPSG * (NSUInteger)n_head_kv * kv_row_bytes;
        const NSUInteger nrows = (NSUInteger)n_head;
        const NSUInteger tmp_bytes =
            nrows * (NSUInteger)head_dim * QWEN_FLASH_NWG * sizeof(float) +
            nrows * (2u * QWEN_FLASH_NWG) * sizeof(float);
        if ((has_kvpad &&
        ...
        if (has_kvpad) {
            ds4_gpu_flash_attn_pad_args pad_args = {
                ...
                .ne_12_3 = 1,
                .nb11 = kv_row_bytes,
                .nb12 = (uint64_t)cap * kv_row_bytes,
                .nb13 = (uint64_t)cap * n_head_kv * kv_row_bytes,
                .nb21 = kv_row_bytes,
                .nb22 = (uint64_t)cap * kv_row_bytes,
                .nb23 = (uint64_t)cap * n_head_kv * kv_row_bytes,
                .ne31 = 1,
                .ne32 = 1,
                .ne33 = 1,
                .nb31 = 0,
            ...
            [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:1];
            [enc setBuffer:vbuf offset:ds4_gpu_tensor_offset(v_cache) atIndex:2];
            [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:3];
            [enc setBuffer:g_flash_attn_pad_buffer offset:0 atIndex:4];
            [enc dispatchThreadgroups:MTLSizeMake(QWEN_FLASH_NCPSG, n_head_kv, 1)
        ...
        ds4_gpu_flash_attn_vec_args args = {
            ...
            .nb02 = q_row_bytes,
            .nb03 = (uint64_t)n_head * q_row_bytes,
            .ne11 = (int32_t)n_ctx,
            .ne_12_2 = (int32_t)n_head_kv,
            .ne_12_3 = 1,
        ...
        const NSUInteger shared_elems =
            ((NSUInteger)head_dim +
             4u * QWEN_FLASH_NCPSG +
             2u * (NSUInteger)head_dim) * nsg;
        const NSUInteger shared_bytes =
            (shared_elems * (sizeof(float) / 2u) + 15u) & ~(NSUInteger)15u;

        id<MTLComputeCommandEncoder> enc = ds4_gpu_compute_encoder(cb);
        [enc setComputePipelineState:flash_pipeline];
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        [enc setBuffer:qbuf offset:ds4_gpu_tensor_offset(q) atIndex:1];
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:2];
        [enc setBuffer:vbuf offset:ds4_gpu_tensor_offset(v_cache) atIndex:3];
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:4];
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:5];
        ...
        [enc dispatchThreadgroups:MTLSizeMake(1, n_head, QWEN_FLASH_NWG)
        ...
        [enc setBuffer:hbuf offset:ds4_gpu_tensor_offset(heads) atIndex:2];
        [enc dispatchThreadgroups:MTLSizeMake(nrows, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(32u * QWEN_FLASH_NWG, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen attention")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_rope_tensor(
        ...
        float             freq_base) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!x || n_head == 0 || head_dim == 0 || (head_dim & 1u) != 0) return 0;
    @autoreleasepool {
        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        const uint64_t bytes = (uint64_t)n_head * head_dim * sizeof(float);
        ...
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        ds4_gpu_rope_tail_batch_args args =
            ds4_gpu_make_rope_tail_args(1, n_head, head_dim, head_dim,
                                        0, false, freq_base, 1.0f,
                                        0.0f, 1.0f, 32.0f, 1.0f);
        args.mode = 2;
        if (!ds4_gpu_encode_rope_tail_inplace(cb, xbuf, ds4_gpu_tensor_offset(x),
                                              &args, 1, n_head, head_dim, pos, 1)) {
            return 0;
        }
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen RoPE")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_rope_batch_tensor(
        ...
        float             freq_base) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!x || n_tok == 0 || n_head == 0 || head_dim == 0 || (head_dim & 1u) != 0) return 0;
    @autoreleasepool {
        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        const uint64_t bytes = (uint64_t)n_tok * n_head * head_dim * sizeof(float);
        if (!xbuf || ds4_gpu_tensor_bytes(x) < bytes) return 0;
        ...
        if (!cb) return 0;
        ds4_gpu_rope_tail_batch_args args =
            ds4_gpu_make_rope_tail_args(n_tok, n_head, head_dim, head_dim,
                                        0, false, freq_base, 1.0f,
                                        0.0f, 1.0f, 32.0f, 1.0f);
        args.mode = 2;
        if (!ds4_gpu_encode_rope_tail_inplace(cb, xbuf, ds4_gpu_tensor_offset(x),
                                              &args, n_tok, n_head, head_dim, pos0, 1)) {
            return 0;
        }
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen batch RoPE")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_attention_batch_tensor(
        ...
        uint32_t                head_dim) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!heads || !q || !k_cache || !v_cache || n_tokens == 0 || cap == 0 ||
        pos0 > cap || n_tokens > cap - pos0 ||
        n_head == 0 || n_head_kv == 0 || head_dim == 0) return 0;
    const uint64_t q_bytes = (uint64_t)n_tokens * n_head * head_dim * sizeof(float);
    const uint64_t kv_bytes = (uint64_t)cap * n_head_kv * head_dim * sizeof(uint16_t);
    @autoreleasepool {
        ...
        if (!cb) return 0;
        NSUInteger nth = head_dim < 256u ? head_dim : 256u;
        ds4_gpu_qwen_attention_batch_args args = {
            .pos0 = pos0,
            .n_tokens = n_tokens,
            .cap = cap,
            .n_head = n_head,
        ...
        [enc setComputePipelineState:pipeline];
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        [enc setBuffer:hbuf offset:ds4_gpu_tensor_offset(heads) atIndex:1];
        [enc setBuffer:qbuf offset:ds4_gpu_tensor_offset(q) atIndex:2];
        [enc setBuffer:kbuf offset:ds4_gpu_tensor_offset(k_cache) atIndex:3];
        [enc setBuffer:vbuf offset:ds4_gpu_tensor_offset(v_cache) atIndex:4];
        [enc setThreadgroupMemoryLength:2u * nth * sizeof(float) atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake(n_head, n_tokens, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen batch attention")) return 0;
    }
    return 1;
}
...
static id<MTLComputePipelineState> ds4_gpu_routed_mv_pipeline(uint32_t type) {
    ...
    case DS4_METAL_TENSOR_Q6_K:    return g_moe_mul_mv_id_q6_k_pipeline;
    default:                       return nil;
    }
}
...
static int ds4_gpu_encode_mul_mv_id_sum6(
    ...
    const NSUInteger rows_per_group = (NSUInteger)args->nr0;
    const NSUInteger row_groups = ((NSUInteger)args->ne01 + rows_per_group - 1u) / rows_per_group;
    const NSUInteger partial_bytes = 8u * (NSUInteger)args->nr0 * sizeof(float);
...
static int ds4_gpu_encode_mul_mv_id_sum8(
        id<MTLCommandBuffer>        cb,
        id<MTLComputePipelineState> pipeline,
        const ds4_gpu_mul_mv_id_args *args,
        id<MTLBuffer>               src0,
...
static int ds4_gpu_encode_moe_swiglu_weight(
    ...
    ds4_gpu_dsv4_moe_swiglu_weight_args args = {
        ...
        .write_clamped = ds4_gpu_moe_write_clamped_act() ? 1u : 0u,
...
int ds4_gpu_qwen_router_select_tensor(
        ds4_gpu_tensor       *selected,
        ds4_gpu_tensor       *weights,
        ds4_gpu_tensor       *probs,
        const ds4_gpu_tensor *logits,
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> wbuf = ds4_gpu_tensor_buffer(weights);
        id<MTLBuffer> pbuf = ds4_gpu_tensor_buffer(probs);
        id<MTLBuffer> lbuf = ds4_gpu_tensor_buffer(logits);
        if (!sbuf || !wbuf || !pbuf || !lbuf ||
            ...
            ds4_gpu_tensor_bytes(logits) < (uint64_t)n_expert * sizeof(float)) return 0;
        int owned = 0;
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        ds4_gpu_qwen_router_args args = { .n_expert = n_expert, .n_expert_used = n_expert_used };
        id<MTLComputeCommandEncoder> enc = ds4_gpu_compute_encoder(cb);
        [enc setComputePipelineState:pipeline];
        [enc setBytes:&args length:sizeof(args) atIndex:0];
        ...
        [enc dispatchThreadgroups:MTLSizeMake(1, 1, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen router")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_router_select_batch_tensor(
    ...
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!selected || !weights || !probs || !logits || n_expert == 0 ||
        n_expert_used == 0 || n_expert_used > n_expert || n_tokens == 0) return 0;
    @autoreleasepool {
        id<MTLComputePipelineState> pipeline =
            ds4_gpu_get_pipeline("kernel_qwen_router_topk_softmax_batch_f32");
        if (!pipeline) return 0;
        id<MTLBuffer> sbuf = ds4_gpu_tensor_buffer(selected);
        id<MTLBuffer> wbuf = ds4_gpu_tensor_buffer(weights);
        id<MTLBuffer> pbuf = ds4_gpu_tensor_buffer(probs);
        ...
        if (!sbuf || !wbuf || !pbuf || !lbuf ||
            ...
            ds4_gpu_tensor_bytes(logits) < (uint64_t)n_tokens * n_expert * sizeof(float)) return 0;
        int owned = 0;
        id<MTLCommandBuffer> cb = ds4_gpu_command_buffer(&owned);
        if (!cb) return 0;
        ds4_gpu_qwen_router_args args = { .n_expert = n_expert, .n_expert_used = n_expert_used };
        ...
        [enc dispatchThreads:MTLSizeMake(n_tokens, 1, 1)
         threadsPerThreadgroup:MTLSizeMake(MIN((NSUInteger)n_tokens, (NSUInteger)256), 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen batch router")) return 0;
    }
    return 1;
}
...
int ds4_gpu_routed_moe_one_tensor(
    ...
    @autoreleasepool {
        ...
        const uint32_t total_experts = 128u;
        const uint64_t gate_tensor_bytes = (uint64_t)total_experts * gate_expert_bytes;
        const uint64_t down_tensor_bytes = (uint64_t)total_experts * down_expert_bytes;
        uint64_t gate_inner = 0;
        uint64_t up_inner = 0;
        ...
        ds4_gpu_mul_mv_id_args gate_args =
            ds4_gpu_make_mul_mv_id_args(expert_in_dim, expert_mid_dim, total_experts,
                                          gate_row_bytes, gate_expert_bytes,
                                          1, n_expert, n_tokens, gate_nr0);
        ds4_gpu_mul_mv_id_args down_args =
            ds4_gpu_make_mul_mv_id_args(expert_mid_dim, out_dim, total_experts,
...
int ds4_gpu_qwen_routed_moe_tensor(
    ...
    if (!out || !gate || !up || !mid || !model_map || !selected || !weights || !x ||
        ...
        expert_in_dim == 0 || expert_mid_dim == 0 || out_dim == 0 || n_expert == 0) return 0;

    @autoreleasepool {
        id<MTLComputePipelineState> gate_pipeline =
            ds4_gpu_get_pipeline("kernel_qwen_moe_gate_up_q4_K");
        id<MTLComputePipelineState> down_pipeline =
            ds4_gpu_get_pipeline(down_type == DS4_METAL_TENSOR_Q6_K
                                 ? "kernel_qwen_moe_down_q6_K"
                                 : "kernel_qwen_moe_down_q4_K");
        if (!gate_pipeline || !down_pipeline) return 0;

        const uint64_t gate_tensor_bytes = (uint64_t)128u * gate_expert_bytes;
        const uint64_t down_tensor_bytes = (uint64_t)128u * down_expert_bytes;
        if (gate_offset > model_size || up_offset > model_size || down_offset > model_size ||
            gate_tensor_bytes > model_size - gate_offset ||
            gate_tensor_bytes > model_size - up_offset ||
            down_tensor_bytes > model_size - down_offset) return 0;

        uint64_t gate_inner = 0;
        uint64_t up_inner = 0;
        uint64_t down_inner = 0;
        id<MTLBuffer> gate_w = ds4_gpu_wrap_model_range(model_map, model_size, gate_offset, gate_tensor_bytes, &gate_inner);
        id<MTLBuffer> up_w = ds4_gpu_wrap_model_range(model_map, model_size, up_offset, gate_tensor_bytes, &up_inner);
        id<MTLBuffer> down_w = ds4_gpu_wrap_model_range(model_map, model_size, down_offset, down_tensor_bytes, &down_inner);
        id<MTLBuffer> xbuf = ds4_gpu_tensor_buffer(x);
        id<MTLBuffer> sbuf = ds4_gpu_tensor_buffer(selected);
        id<MTLBuffer> wbuf = ds4_gpu_tensor_buffer(weights);
        id<MTLBuffer> gatebuf = ds4_gpu_tensor_buffer(gate);
        ...
        id<MTLBuffer> outbuf = ds4_gpu_tensor_buffer(out);
        const uint64_t mid_bytes = (uint64_t)n_expert * expert_mid_dim * sizeof(float);
        if (!gate_w || !up_w || !down_w || !xbuf || !sbuf || !wbuf ||
            !gatebuf || !upbuf || !midbuf || !outbuf ||
            ds4_gpu_tensor_bytes(x) < (uint64_t)expert_in_dim * sizeof(float) ||
        ...
        if (!cb) return 0;
        const NSUInteger nth = 256u;
        ds4_gpu_qwen_moe_args args = {
            .expert_in_dim = expert_in_dim,
            .expert_mid_dim = expert_mid_dim,
            .out_dim = out_dim,
            .n_expert_used = n_expert,
            .gate_row_bytes = (uint32_t)gate_row_bytes,
            .gate_expert_bytes = (uint32_t)gate_expert_bytes,
            .down_row_bytes = (uint32_t)down_row_bytes,
            .down_expert_bytes = (uint32_t)down_expert_bytes,
        };
        ...
        [enc setBuffer:xbuf offset:ds4_gpu_tensor_offset(x) atIndex:3];
        [enc setBuffer:sbuf offset:ds4_gpu_tensor_offset(selected) atIndex:4];
        [enc setBuffer:wbuf offset:ds4_gpu_tensor_offset(weights) atIndex:5];
        [enc setBuffer:gatebuf offset:ds4_gpu_tensor_offset(gate) atIndex:6];
        [enc setBuffer:upbuf offset:ds4_gpu_tensor_offset(up) atIndex:7];
        [enc setBuffer:midbuf offset:ds4_gpu_tensor_offset(mid) atIndex:8];
        [enc setThreadgroupMemoryLength:2u * nth * sizeof(float) atIndex:0];
        [enc dispatchThreadgroups:MTLSizeMake(expert_mid_dim, n_expert, 1)
             threadsPerThreadgroup:MTLSizeMake(nth, 1, 1)];
        ds4_gpu_end_compute_encoder(cb, enc);
        ...
        [enc setBuffer:sbuf offset:ds4_gpu_tensor_offset(selected) atIndex:3];
        [enc setBuffer:outbuf offset:ds4_gpu_tensor_offset(out) atIndex:4];
        [enc setThreadgroupMemoryLength:nth * sizeof(float) atIndex:0];
        ...
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen routed MoE")) return 0;
    }
    return 1;
}
...
int ds4_gpu_qwen_routed_moe_batch_tensor(
    ...
    if (!out || !gate || !up || !mid || !model_map || !selected || !weights || !x ||
        ...
        n_expert == 0 || n_tokens == 0) return 0;

    @autoreleasepool {
        id<MTLComputePipelineState> gate_pipeline =
            ds4_gpu_get_pipeline("kernel_qwen_moe_gate_up_q4_K_batch");
        id<MTLComputePipelineState> down_pipeline =
            ds4_gpu_get_pipeline(down_type == DS4_METAL_TENSOR_Q6_K
                                 ? "kernel_qwen_moe_down_q6_K_batch"
                                 : "kernel_qwen_moe_down_q4_K_batch");
        if (!gate_pipeline || !down_pipeline) return 0;
        ...
        if (!gate_w || !up_w || !down_w || !xbuf || !sbuf || !wbuf ||
            !gatebuf || !upbuf || !midbuf || !outbuf ||
            ds4_gpu_tensor_bytes(x) < (uint64_t)n_tokens * expert_in_dim * sizeof(float) ||
            ds4_gpu_tensor_bytes(selected) < (uint64_t)n_tokens * n_expert * sizeof(int32_t) ||
            ds4_gpu_tensor_bytes(weights) < (uint64_t)n_tokens * n_expert * sizeof(float) ||
            ds4_gpu_tensor_bytes(gate) < mid_bytes ||
            ds4_gpu_tensor_bytes(up) < mid_bytes ||
            ds4_gpu_tensor_bytes(mid) < mid_bytes ||
            ds4_gpu_tensor_bytes(out) < (uint64_t)n_tokens * out_dim * sizeof(float)) return 0;
        ...
        if (!ds4_gpu_finish_command_buffer(cb, owned, "Qwen batch routed MoE")) return 0;
    }
    return 1;
}
...
int ds4_gpu_routed_moe_batch_tensor(
    ...
    @autoreleasepool {
        ...
        if (use_mm_id) {
            gate_map_args =
                ds4_gpu_make_mul_mm_id_map_args(expert_in_dim, total_experts, 1, n_expert, n_tokens);
            gate_mm_args =
                ds4_gpu_make_mul_mm_id_args(expert_in_dim, expert_mid_dim, total_experts,
                                              gate_row_bytes, gate_expert_bytes,
                                              1, n_expert, n_tokens);
            down_mm_args =
                ds4_gpu_make_mul_mm_id_args_src1_size(expert_mid_dim, out_dim, total_experts,
                                                        down_row_bytes, down_expert_bytes,
